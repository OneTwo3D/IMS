...
const DETAIL_FIELDS: FieldDef[] = [
  ...
  { key: 'customerName', label: 'Customer', type: 'text' },
  { key: 'salesRep', label: 'Sales Rep', type: 'text' },
  { key: 'status', label: 'Status', type: 'select', options: ['DRAFT', 'PENDING_PAYMENT', 'ON_HOLD', 'PROCESSING', 'ALLOCATED', 'PICKING', 'PACKING', 'SHIPPED', 'COMPLETED', 'DELIVERED', 'CANCELLED'] },
  { key: 'qty', label: 'Qty', type: 'number' },
  { key: 'totalBase', label: 'Total', type: 'number' },
...
const DEFAULT_COLS: Record<Tab, string[]> = {
  ...
  details: ['productName', 'sku', 'barcode', 'mpn', 'customerName', 'salesRep', 'status', 'qty', 'totalBase', 'createdAt'],
        ...
        <div className="flex items-center gap-1 overflow-x-auto overflow-y-hidden">
          {TABS.map((t) => (
            <button key={t.key} type="button"
              className={`shrink-0 px-3 sm:px-4 py-2 text-sm font-medium border-b-2 transition-colors -mb-px ${tab === t.key ? 'border-primary text-primary' : 'border-transparent text-muted-foreground hover:text-foreground'}`}
