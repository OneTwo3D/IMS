...
export type PurchasingAnalyticsReportKey = 'open-pos' | 'supplier-performance' | 'ppv' | 'spend' | 'lead-times'

export type PurchasingAnalyticsFilterValues = {
  dateFrom?: string
  dateTo?: string
  pageSize?: string
}

export type PurchasingAnalyticsColumn<Row> = {
  key: string
  label: string
  align?: 'left' | 'right'
...
type PurchasingAnalyticsReportPageProps<Row> = {
  title: string
  description: string
  reportKey: PurchasingAnalyticsReportKey
  filters: PurchasingAnalyticsFilterValues
  pageInfo: PageInfo
  rows: Row[]
  rowKey: (row: Row, index: number) => string
  columns: Array<PurchasingAnalyticsColumn<Row>>
  summary: Array<{ label: string; value: string; tone?: SummaryTone }>
  notices?: string[]
}

export function PurchasingAnalyticsReportPage<Row>({
  title,
  description,
  reportKey,
  filters,
  ...
  summary,
  notices = [],
}: PurchasingAnalyticsReportPageProps<Row>) {
  const params = currentParams(filters)
  const csvHref = `/api/export/purchasing-analytics?${appendParams(params, { report: reportKey })}`
