...
export default async function PurchaseOrderDetailPage({ params }: Props) {
  ...
  const [po, suppliers, productsResult, warehouses, currencies, taxRates, purchaseUnits, billUrlTemplate, organisation] = await Promise.all([
    getPurchaseOrder(id),
    getSuppliers(),
  ...
  return (
    <div className="space-y-4">
      ...
      <PoDetailClient po={po} suppliers={suppliers} products={products} warehouses={warehouses} currencies={currencies} taxRates={taxRates} purchaseUnits={purchaseUnits} companyHomeCountry={organisation?.country ?? null} accountingBillUrlTemplate={billUrlTemplate ?? 'https://go.xero.com/AccountsPayable/View.aspx?InvoiceID={id}'} />
    </div>
  )
}
