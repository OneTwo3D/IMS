...
- **Plugins** — enable or disable shopping/accounting connector plugins. Disabled plugins are hidden from menus and shared UI.
  - Some plugins are **mutually exclusive**: Xero or QuickBooks — never both, and one WMS connector at a time. Saving is refused with "Enable either Xero or QuickBooks, not both" rather than partially applied, and the refusal writes nothing at all, including the plugins in the same save that were individually fine. (There used to be a WooCommerce-or-Shopify pair as well; Shopify was removed in 2026-09, so WooCommerce is now the only shopping connector and has nothing to conflict with.)
