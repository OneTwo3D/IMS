...
- **Tax rates** — define at least one VAT rate (e.g. "UK Standard 20%"). Each rate has a country code and a Xero tax type code for accounting sync. Once your accounting connector is linked, the **Tax rate mapping** card can map your IMS rates to existing connector tax codes (**Auto-apply**) or, for rates the connector is missing, create and map them for you (**Generate missing rates**, Xero only) — see `settings.md`.
