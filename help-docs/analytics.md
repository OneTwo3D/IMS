...
A third case reads differently, because it is repaired differently. Where a dispatch carries COGS entries for **more** units than the movement moved, the cell says `Inconsistent` rather than `Withheld` and the report adds a notice that **names the affected customers**, highest-ranked first — the rows are paginated and the count is not, so a notice that only counted could be talking about a customer pages away from the one on screen. Each one is named as `name="…" email="…" customerId="…" group="…"`, because a name on its own is not an identifier: two guests can share one, and the stored name can be empty. The first three fields are the Customer cell, the Email cell and the `customerId` column of the CSV export; `group=` is the identity the report actually grouped that row under, so two rows that match on the first three are still told apart. A registered customer with no stored email is named `name="Acme Ltd" email=<none> customerId="cust-1" group="cust-1"`, and an emailless guest of the same name is `name="Acme Ltd" email=<none> customerId=<none> group="guest-name:Acme Ltd"` — same first two fields, two different rows. Every present value is quoted with any inner quote doubled, so a value containing the `; ` that separates entries cannot break the list, and an *absent* value is the unquoted token `<none>`, which no present value can wear because a present value is always quoted. Whitespace and invisible characters inside a value are written out as escapes — `\t`, `\n`, `\u00a0` for a non-breaking space, `\u0020` for a second consecutive space, and a literal backslash doubled — because the notice is rendered as HTML, where a tab, a newline and a run of spaces all collapse to a single space on screen and two customers whose names differ only there would otherwise reach you as one name. `group=` goes further and is **ASCII only**: every character outside printable ASCII is written out as its code point, so an emailless guest called 株式会社アクメ is named `name="株式会社アクメ" email=<none> customerId=<none> group="guest-name:\u682a\u5f0f\u4f1a\u793e\u30a2\u30af\u30e1"`. That is deliberate, and it is the reason the notice carries both a readable half and an identity half: two spellings of one accented name, or a Greek and a Cyrillic capital A, are different rows that any screen draws identically, and an identity you cannot tell apart is not an identity. Find the row by reading `name=` and `email=`, which show exactly what the Customer and Email cells show; use `group=` to tell two otherwise identical rows apart, not to read a name off. Past ten names it says how many more there are and points at that export, which is not paginated and carries `costEvidence` on every row. Any excess at all counts, down to the smallest quantity the database can store: the costed-quantity match allows a rounding shortfall, because the FIFO engine leaves one, but nothing above the movement, because the engine cannot produce it. FIFO consumption can never cost more than it moved, so this is not a gap that a later shipment fills — it is contradictory evidence, and it will still be there next period unless somebody corrects the entries.
