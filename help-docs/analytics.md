...
Figures that carry a **≤** are rounded **upward** and figures that carry a **≥** are rounded **downward**, on the page and in the CSV alike, so a printed figure is never on the wrong side of the relation it states; figures with no mark or a **?** are rounded to nearest. On the page every amount is rounded once, to the number of decimals your base currency is printed with — none for yen, three for Kuwaiti dinars — so the rounding you see is the only rounding applied. That rounding is done on the exact stored figure and never on a converted one, which matters at the top of the range: the gap between the numbers a computer can hold as fractions is wider than a hundredth once an amount passes about 90,000,000,000,000, so a figure converted before being rounded could land on the wrong side of its own bound. Because each row and each total is rounded on its own, a column of rows may not add up exactly to the total beneath it, and on a report with more than one page the total also covers the rows on the other pages. The COGS Report says so in its notices.
