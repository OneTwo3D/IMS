...
Figures that carry a **≤** are rounded **upward** to the displayed precision, on the page and in the CSV alike, so the printed number is never below the figure it claims to be at or above; a **≥** is rounded **downward** for the mirror reason. That rounding is done on the exact stored figure and never on a converted one, which matters at the top of the range: the gap between the numbers a computer can hold as fractions is wider than a penny once an amount passes about £90,000,000,000,000, so a figure converted before being rounded could land a penny the wrong side of its own bound. One consequence looks like an error and is not: a column of rounded rows will generally **not** add up to the rounded total beneath it — it can come out slightly above or slightly below, by up to a penny per row — because the total is rounded once from the unrounded sum rather than from the rows on screen. Both numbers are sound; making them tally is what would make one of them false. The COGS Report states this on the page, and its CSV repeats it as a `roundingReconciliation` comment row, so the explanation reaches you beside the figures rather than only here.
