...
A third case reads differently, because it is repaired differently. Where a dispatch carries COGS entries for **more** units than the movement moved, the cell says `Inconsistent` rather than `Withheld` and the report adds a notice that **names the affected customers**, highest-ranked first — the rows are paginated and the count is not, so a notice that only counted could be talking about a customer pages away from the one on screen. Each one is named as `"customer name" email [customer id]`, because a name on its own is not an identifier: two guests can share one, and the stored name can be empty. The name is quoted so that a name containing a semicolon does not break the list, the email is the one in the Email column, and the id is a column of the CSV export — a guest with no customer record shows `[guest]`. Past ten names it says how many more there are and points at that export, which is not paginated and carries `costEvidence` on every row. Any excess at all counts, down to the smallest quantity the database can store: the costed-quantity match allows a rounding shortfall, because the FIFO engine leaves one, but nothing above the movement, because the engine cannot produce it. FIFO consumption can never cost more than it moved, so this is not a gap that a later shipment fills — it is contradictory evidence, and it will still be there next period unless somebody corrects the entries.

**Read what that notice claims carefully: it is a statement about quantities.** The check compares the costed quantity with the quantity that moved, and never looks at the money. Entries posted twice are the usual explanation, and where that is the explanation every figure anywhere that sums COGS entries is overstating cost by the duplicate for as long as it stands — but it is not the only explanation. The same excess quantity can sit on entries costed at zero, or on a malformed quantity beside a perfectly correct total, and in neither of those is a penny of cost overstated. Nor can the report work out the overstatement for you: it cannot tell which entries are the spurious ones, so there is no proven excess figure it could publish. Open the dispatch's COGS entries and compare them with the movement before you change any posted cost.
