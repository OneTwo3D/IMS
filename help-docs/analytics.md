...
Two details worth knowing before you reconcile against it. The credit is anchored to the date the refund was **raised**, which is the same rule the Gross Margin report uses: a credit raised this month against last month's dispatch reduces this month's revenue, exactly as a credit note does to a month's accounts, and a credit raised next month is not in this month's figures at all. And the credit loaded is only credit against the orders behind **this** report's dispatches — a credit against an order the report does not cover could never reach one of its rows, and counting it would fill the off-report totals with credit your own filters excluded.
