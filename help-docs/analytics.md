...
Figures that carry a **≤** are rounded **upward** to the displayed precision, on the page and in the CSV alike, so the printed number is never below the figure it claims to be at or above. One consequence looks like an error and is not: a column of rounded-up rows can add up to slightly more than the rounded-up total beneath it, because the total is rounded once from the unrounded sum rather than from the rows on screen. Both numbers are sound; making them tally is what would make one of them false.
