...
One path is **not** covered: the unconfirmed-backend branch of the restore endpoint deliberately leaves maintenance mode on, and an operator clears the flag directly in the `settings` table (there is no UI). A window ended that way leaves no stamp, so its refused callbacks fall back to the watchdog alert and the manual Re-check.
