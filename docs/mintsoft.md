...
- **Duplicate recovery before an ASN push reads the whole ASN list, or refuses (o3d-bhvu).** Before creating an ASN, both creators look for one an earlier attempt may already have created. They read `GET /api/ASN/List`, not `GET /api/ASN`: live Mintsoft answers that with 405, because it is the create route.
  ...
  - **How an ASN counts as the same one.** The reference must equal Mintsoft's `POReference` exactly (surrounding whitespace ignored; `PO-1` never matches `PO-10`). It must have the same number of items, and each line's `SourceLineId` must be present at its expected quantity. Matching on `SourceLineId` assumes Mintsoft stores and returns IMS's line id unchanged, which is not yet confirmed live (o3d-vcw8).
