...
- **Duplicate recovery before an ASN push reads the whole ASN list, or refuses (o3d-bhvu).** Before creating an ASN, both creators look for one an earlier attempt may already have created. They read `GET /api/ASN/List`, not `GET /api/ASN`: live Mintsoft answers that with 405, because it is the create route.
  ...
  - **What recovery does NOT look at (o3d-54al keeps this).** The ASN's **status** — a COMPLETE, booked-in ASN with the same reference, line ids and quantities is still adopted as though it were this attempt's, and the attempt then waits for a booked-in callback that has already happened. Round 7 landed the other half of o3d-54al (the already-mapped check) because the Codex HIGH needed it; status is orthogonal and needs its own decision about which `ASNStatusId`s may be adopted at all. Two narrower cases are also left, deliberately: a **rounding-explainable** quantity and a **warehouse mismatch** refuse whether or not the ASN is mapped, because their content is indistinguishable from (or identical to) what is about to be sent, so nothing has shown a different consignment; and if *every* line of a purchase order were deleted and re-added between a lost create and the retry, the earlier ASN would share none of the new line ids and would read as unrelated.
- **ASN creation, the contract Mintsoft actually accepts (o3d-vcw8, proven live 2026-09-24).** The create is `PUT /api/ASN` — there is no `POST /api/ASN`; the swagger's POST route is the UPDATE (`POST /api/ASN/{id}`), which is why live `GET /api/ASN` answers 405. The body is a `NewASN`: `WarehouseId`, `POReference` (**not** `Reference`), `Items` (**not** `Lines`, each with `SourceLineId`, `ProductId`, `SKU`, `Quantity`), `EstimatedDelivery` (**not** `ETA`), `GoodsInType` and a header `Quantity`.
  ...
  - A new ASN lands on `ASNStatusId 1`/`NEW`. `ASNStatus` is an OBJECT and carries **no** `ExternalName` — that trick is order-only — so ASN state must be derived from `ASNStatusId`/`Name` (o3d-btiw).
