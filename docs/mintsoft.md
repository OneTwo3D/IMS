...
- **Known gap: ASN creation itself does not yet match Mintsoft's published contract (o3d-vcw8).** Creation is `PUT /api/ASN` with a `NewASN` body (`POReference`, `Items`) returning a `ToolkitResult` — proven live on 2026-09-24 with one owner-sanctioned test ASN, created and then deleted. IMS still sends `POST /api/ASN` with `Reference`/`Lines`, which the swagger does not define, so an ASN push against live Mintsoft fails at the push and the reservation stays `CREATE_PENDING`. What that one create established, and what the fix must honour: `ClientId` must be **omitted** (a client-user key is refused outright if it is sent), `GoodsInType` is **required** (the swagger does not mark it so), a new ASN lands on `ASNStatusId 1`/`NEW`, `DELETE /api/ASN/{id}` is the only removal, and **every failure comes back as HTTP 200 with `Success: false`** — so the client must require `Success === true` and `ID > 0` rather than trusting the status code. Never test ASN creation against the live tenant without explicit owner sign-off: Mintsoft fulfils what it is sent.
