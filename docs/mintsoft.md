...
- **Duplicate recovery before an ASN push reads the whole ASN list, or refuses (o3d-bhvu).** Before creating an ASN, both creators look for one an earlier attempt may already have created. They match on Mintsoft's `POReference` and on each item's `SourceLineId` and expected quantity. The list is `GET /api/ASN/List`, not `GET /api/ASN`: live Mintsoft answers `GET /api/ASN` with 405, because that path is the create route. The list pages at 100: `Limit` above 100 is an HTTP 400, and its row order is not stable across pages. So IMS reads it page by page with `Limit=100` and `IncludeASNItems=true` (scoped to the ASN's warehouse), stops at the first short page, and gives up after 50 pages. It accepts a read only when two consecutive full reads return exactly the same set of ASN IDs. Anything less, or any failed page, **aborts the attempt without creating anything**: a partial list read as "no such ASN" would create a duplicate at a live warehouse. The aborted attempt is left `CREATE_PENDING` and can be retried.
