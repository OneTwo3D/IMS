...
- **Duplicate recovery before an ASN push reads the whole ASN list, or refuses (o3d-bhvu).** Before creating an ASN, both creators look for one an earlier attempt may already have created. They read `GET /api/ASN/List`, not `GET /api/ASN`: live Mintsoft answers that with 405, because it is the create route.
  ...
  - **What recovery does NOT look at (o3d-54al).** The ASN's status — a COMPLETE, booked-in ASN with the same reference and line ids is adopted as though it were this attempt's — and whether IMS has already mapped that ASN id, which then fails on the `@@unique(connector, externalAsnId)` constraint after the decision. Also: an ASN whose quantity was edited at the warehouse by more than rounding reads as "not ours", and a second ASN is created.
- **ASN creation, the contract Mintsoft actually accepts (o3d-vcw8, proven live 2026-09-24).** The create is `PUT /api/ASN` — there is no `POST /api/ASN`; the swagger's POST route is the UPDATE (`POST /api/ASN/{id}`), which is why live `GET /api/ASN` answers 405. The body is a `NewASN`: `WarehouseId`, `POReference` (**not** `Reference`), `Items` (**not** `Lines`, each with `SourceLineId`, `ProductId`, `SKU`, `Quantity`), `EstimatedDelivery` (**not** `ETA`), `GoodsInType` and a header `Quantity`.
  ...
  - **The create is verified afterwards, not assumed.** The `ToolkitResult` carries an id and nothing else, so IMS reads the ASN back with `GET /api/ASN/{ID}` (which is also where the `ASNItem.ID`/`SourceLineId` line mapping comes from) and requires the `POReference` and every `SourceLineId` it sent to be there. Both round-trip verbatim, so those two are exactly what a later retry finds the ASN by. If the read-back does not confirm them, nothing is recorded and the error names the ASN id: the ASN exists at the warehouse and only an operator can remove it — `DELETE /api/ASN/{id}` is Mintsoft's only removal (there is no cancel route and no cancelled status).
