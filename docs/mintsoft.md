...
- **Duplicate recovery before an ASN push reads the whole ASN list, or refuses (o3d-bhvu).** Before creating an ASN, both creators look for one an earlier attempt may already have created. They read `GET /api/ASN/List`, not `GET /api/ASN`: live Mintsoft answers that with 405, because it is the create route.
  - **How the list is read.** The list pages at 100 (`Limit` above 100 is an HTTP 400) and its row order is not stable across pages. IMS reads it page by page with `Limit=100` and `IncludeASNItems=true` **across the whole tenant**, not just the ASN's warehouse, stops at the first short page, and gives up after 50 pages. It accepts a read only when two consecutive full reads return exactly the same set of ASN IDs.
  - **How an ASN counts as the same one.** The reference must equal Mintsoft's `POReference` exactly (surrounding whitespace ignored; `PO-1` never matches `PO-10`). It must carry the same number of items **as Mintsoft returned them** — an item IMS cannot read, because it has no string `SourceLineId`, still counts, so an ASN with an extra item is not this one — and each reservation line must be present by `SourceLineId` at its expected quantity. Matching on `SourceLineId` assumes Mintsoft stores and returns IMS's line id unchanged, which is not yet confirmed live (o3d-vcw8).
