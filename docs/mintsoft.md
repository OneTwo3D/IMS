...
- **Duplicate recovery before an ASN push reads the whole ASN list, or refuses (o3d-bhvu).** Before creating an ASN, both creators look for one an earlier attempt may already have created. They read `GET /api/ASN/List`, not `GET /api/ASN`: live Mintsoft answers that with 405, because it is the create route.
  ...
  - **A quantity that merely DIFFERS is an unresolved conflict, not a different ASN (o3d-bhvu round 6).** A row carrying this reference and *exactly* these `SourceLineId`s whose `QuantityExpected` differs by more than rounding could explain used to be answered with "no such ASN exists" — and the creator pushed a second inbound ASN at a live warehouse for lines the first already covered. That state is what an ASN a lost create really made looks like once the source quantity has changed (the reservation checks validate the CURRENT LOCAL quantities, never whether the earlier remote ASN exists); it is also what an operator's edit at the warehouse looks like, and what a later partial over the same lines looks like. Nothing available to the matcher tells them apart, so it is refused by name, naming the ASN, the line and both figures. **Operationally this blocks a second, legitimate partial ASN over the same source lines** until an operator reconciles the first one in Mintsoft — the discriminator that would resolve it (has IMS already mapped that ASN id?) is o3d-54al.
  - **The residual this leaves, stated plainly.** A row carrying this reference whose line set merely *overlaps* the reservation's — the ASN at the warehouse holds `{a,b}`, the retry covers `{a,b,c}` because a line was added to the order — is still read as a different ASN, and one is created. That is the same shape as the quantity case, and it is not refused for one reason: a second ASN over a different set of the same order's lines is how a partial delivery works at all (the reference is the *order's*, shared by every ASN for it), so refusing an overlap would mean no order could have two ASNs without an operator. o3d-54al (ignore an ASN id IMS has already mapped) is what resolves both.
  ...
  - **What recovery does NOT look at (o3d-54al).** The ASN's status — a COMPLETE, booked-in ASN with the same reference and line ids is adopted as though it were this attempt's — and whether IMS has already mapped that ASN id, which then fails on the `@@unique(connector, externalAsnId)` constraint after the decision. As of round 6 an ASN whose quantity was edited at the warehouse no longer reads as "not ours" — it refuses — so what o3d-54al now buys is the opposite: telling an ASN IMS has ALREADY MAPPED apart from one a lost create left behind, so that a legitimate later partial over the same lines does not need an operator.
