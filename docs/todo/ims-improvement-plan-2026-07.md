...
| B10 | B3 follow-up: **double reversal was still possible under concurrency.** Both credit-note paths (WC refund webhook, poller chargeback) decided "already reversed?" from reads taken *outside* the refund transaction, so neither could see the other's uncommitted row; a Xero payment removal + a WC refund inside one poll cycle made both post a credit note, and the refund-total cap did not catch it (chargeback lines are NET, `so.totalBase` is GROSS). **FIXED**: the decision is re-taken inside `createSalesOrderRefund`'s transaction, under the advisory + `sales_orders` row lock it already holds, and the loser returns a typed `conflict` (clean no-op + operator WARNING) instead of an error or a dead-lettered sync. Not a unique index — refunds are legitimately many-per-order and the racing rows share no colliding key | `refund-service.ts` (cross-path guard); `app/actions/sales.ts` (`raiseChargebackForReversedOrder`); `woocommerce/sync/refund-sync.ts` | **6oyu.18 (P3)** ✅ |
...
>
> **B8 reopened and re-fixed (cancelled dispatches).** The first fix filtered the transfer-snapshot query to `status IN ('IN_TRANSIT','RECEIVED')` on the reading that `STOCK_TRANSFER_TRANSITIONS` allows only `IN_TRANSIT -> RECEIVED`, so `CANCELLED` implied "never dispatched". That reading was already wrong: `cancelDispatchedTransfer` (audit-C5, merged 2026-06-13, a month before the fix) performs `IN_TRANSIT -> CANCELLED` **outside** the state machine and explicitly does **not** un-consume the original source layers — it books equivalent replacement layers at the source and links each back with a `costLayerSourceLine`. So every cancelled dispatch kept the original layer at `remainingQty 0` with nothing subtracting it, while `propagateLandedCostToOutputs` revalued the replacement layer through that link: the original 6oyu.19 double count (spurious COGS + transit drained twice), reached through the cancel path instead of the receipt path. Fixed by binding the query's status list from a total `Record<StockTransferStatus, ...>` classification (`STOCK_TRANSFER_SOURCE_LAYER_CONSUMPTION` in `movement-cogs-relevance.ts`) passed as a bound parameter, so the SQL cannot restate it; `CANCELLED` is classified `OUTSTANDING`. In the same change the five per-layer exclusion queries were collapsed into one loader (`loadLayerConsumptionExclusions`) shared by all three revaluation entry points, and the exclusion fields on `CostLayerAdjustmentInput` were made **required** — an omitted exclusion previously defaulted to "nothing to exclude", the permissive answer.
>
> **B8 round 2 — in-transit transfers were stranded, and two WMS paths never linked their layers (Codex review, 2026-09-09).** Two findings, one defect: the registry's contract assumed a `costLayerSourceLine` that two code paths never created, and a third state has no such layer to link to at all.
>
> *(1) WMS-created transfer layers were unreachable.* `booked-in-service.ts` (Mintsoft webhook receipt) and `mintsoft/sync/stock-sync.ts` (stock-sync alignment) created the destination layer and called `copyCostLayerSourceLinesProportionally` **ignoring its return value**. That helper returns 0 for an ordinary PO-derived source layer — it has no `sourceLines` of its own — so, unlike the manual receipt and cancellation paths, no fallback direct link was written and the layer had no `costLayerSourceLine` at all. The snapshot query removed those units from COGS while `getDependentOutputSourceLines` could not find the destination, so the landed-cost delta reached neither COGS nor inventory. Fixed by `lib/domain/inventory/transfer-cost-layer-recreation.ts`: all four paths now go through one helper that guarantees the link as a **postcondition** (it counts the links and throws rather than leave an unreachable layer behind), with a source census test pinning the caller set.
>
> *(2) An `IN_TRANSIT` revaluation had no eventual GL completion path.* `OUTSTANDING` justified the COGS exclusion by "the delta reaches a replacement/destination layer" — true for `RECEIVED` and `CANCELLED`, false for `IN_TRANSIT`, where no layer exists yet. The recalc therefore subtracted the whole snapshot from COGS, propagated into nothing, rewrote the snapshot and queued **no journal**, leaving the freight debit in transit and inventory understated indefinitely. `IN_TRANSIT` now has its own classification, `OUTSTANDING_AWAITING_DESTINATION_LAYER` (the `Record` is total, so every reader had to handle it), and such a revaluation persists a `PendingTransferLandedCostReclass` that the receipt or dispatch cancellation settles — queuing the DR Inventory / CR transit journal and its transit-subledger leg in the **same transaction** that creates the layer.
>
> **Correction to the previous residual note.** It claimed the stranded balance was "visible" because 6oyu.4's `STOCK_IN_TRANSIT` sweep would flag it. It would not, and this is worth remembering as a defect shape: that sweep compares recorded transit *movements* against GL *movements* over a window, and a **missing** posting writes neither a `transit_subledger_movements` row nor a GL line. It is absent from both sides, so the window ties out exactly. A reconciliation between two ledgers can only find a posting that landed in one of them — never one that landed in neither.
