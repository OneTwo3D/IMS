...
- **Fix:**
  ...
  6. Add a `getProductIncomingStock(productId)` helper that returns a structured breakdown by source. The minimum source contract is:
     ...
     - Inbound stock transfers: `stock_transfer_lines` where the parent `stock_transfers.status` is `IN_TRANSIT`, `productId` matches, and the line's OUTSTANDING quantity is above zero. Outstanding is `qty` less the line's LANDED quantity, from `transferLineOutstandingQty` / `loadTransferLineLandedQty` in `lib/domain/inventory/transfer-landed-quantity.ts` — **not** `qty - qtyReceived` (6oyu.19, Codex round-6 HIGH-1): a WMS stock-sync alignment lands units and credits `wms_asn_line_maps.qtyAccountedViaSnapshot` without touching `qtyReceived`, so the raw subtraction counts those units as still inbound and double-counts them against the WMS evidence source listed below.
