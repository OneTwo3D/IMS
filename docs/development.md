...
| **The ONE exemption: the DECLARED database endpoint** | The suite is *configured* with a database, and in CI that database is a service container on a Docker **bridge** address, not loopback — `db-backed-regressions` re-points `DATABASE_URL` at it and the concurrency job re-points `DATABASE_SESSION_LOCK_URL` at it. The trap parses the postgres URLs the environment declares (`DATABASE_URL`, `DIRECT_URL`, `SHADOW_DATABASE_URL`, `DEPLOY_ADMIN_DATABASE_URL`, `DATABASE_SESSION_LOCK_URL`, `UNIX_SOCKET_DATABASE_URL`) **once, at load**, and permits exactly the `(host, port)` pairs they name. Nothing else: a different port on the same declared host is refused, and an **absent** URL permits **nothing** — "no `DATABASE_URL`" can never become "allow everything". It is not a heuristic (`port === 5432`) and not a hardcoded bridge range; change the configured host and the permitted host moves with it, which `tests/no-outbound-network.test.ts` asserts by doing exactly that. Round 9 wired the trap into the two DB tiers without this and turned two CI jobs red (o3d-bhvu round 10). |
| **NOT covered: UDP and raw `dgram`** | DNS lookups still resolve — a lookup sends a name, not our data. |
| **Why it is `.cjs`, and why it carries an `eslint-disable`** | `--require` is the only thing that installs anything in a worker thread, it cannot load ESM on the Node versions `package.json` allows, and a `.ts` trap would need `tsx` resolvable in every child — so the file is CommonJS and uses `require`. It is the only `.cjs` file in the repository, so rather than exempting `**/*.cjs` in `eslint.config.mjs` it carries a file-scoped `eslint-disable @typescript-eslint/no-require-imports` with the reason. Without it `npm run lint` — and therefore `npm run validate` and CI's `validate` job — is RED on five errors (o3d-bhvu round 10; it had been red since the trap was added, because rounds 2-9 ran `check:all` and `build` but never `validate`). |
...
* **ENFORCED, for `RUN_DB_RETENTION_TESTS` ALONE.** `test:db` sets it together with
  ...
  written out by hand in each of those three files. It says nothing about any other gate.
