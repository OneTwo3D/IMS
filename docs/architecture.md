...
**Stock position reports** — `/analytics/stock-on-hand`, `/analytics/stock-allocations`, and `/analytics/negative-stock` share `lib/domain/inventory/stock-position-reports.ts` and are visible to `ADMIN`, `MANAGER`, `WAREHOUSE`, and `FINANCE`. Stock on hand uses `getOnHandAsOf` for quantity/value and enriches rows with current `StockLevel.reservedQty`; historical reservation state is not snapshotted, so reserved/available columns are explicitly current-state. Stock allocations uses `reservation-breakdown.ts` and adds an `other` row whenever known source rows do not reconcile to `StockLevel.reservedQty`. Negative stock reconstructs the selected movement window from an opening as-of balance and also includes currently negative `StockLevel` rows even when no movement happened inside the window. CSV exports use the same domain functions with pagination disabled, so exported totals match the on-screen filtered totals.
