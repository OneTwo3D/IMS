...
- **Sales invoices** — AUTHORISED (WC pre-paid) or DRAFT (manual) invoice created at order time, with automatic Xero payment registration. Edits to sales orders that already carry an `accountingInvoiceId` enqueue a Xero `SALES_INVOICE_UPDATE` with a payload-derived idempotency key (same shape as the purchase bill update path). QuickBooks does not support invoice updates and logs a `sales_invoice_update_skipped_unsupported_connector` WARNING instead.
...
- **Tax rate sync** — multi-component IMS `TaxRate` rows enqueue a Xero `TAX_RATE_SYNC` on save. The processor calls Xero's `POST /TaxRates` with the matching `TaxComponents` payload so the VAT return picks up the component breakdown. Xero matches by `Name` so re-syncs are idempotent. QuickBooks has no equivalent API; the trigger logs a `tax_rate_sync_skipped_unsupported_connector` WARNING instead.
