...
| WMS receipt events — dead-lettered | `wms_inbound_receipt_events` `processingStatus=DEAD` | A booked-in webhook exhausted its retries; its goods-in was **never applied** | Reset to `PENDING` (retry ladder restarted) for the webhook sweeper. Distinct from `REQUIRES_REVIEW`, which is approved from the Mintsoft panel |
...
| Dispatch reconciliation — stuck orders | error logs of the latest `DISPATCH_SYNC` sweep | The WMS despatched the order but IMS could not reconcile it (typically no IMS stock to consume); it re-errors every sweep | Read-only — fix the stock position on the order (linked). Tracked for a first-class dead-letter state under bd `onetwo3d-ims-6oyu.2` |
