...
| Integration outbox — stalled parks | `integration_outbox` `PROCESSING` with `lockedAt` older than every declared drain lease, on an operation no worker may reclaim (o3d-8td2 r3/r6) | A row that never *failed* — it was claimed and its holder never came back. Nothing recovers it automatically: the drain will not reclaim it (the operation is declared `unsafe-to-replay`, so a stale lock is not a licence to hand the work to a second worker), the retry ladder does not apply to a non-failed status, and no reconcile drains the outbox. For `woocommerce/stock.push` every later change to the same product is folded into the parked row and waits behind it. The threshold is derived — `max(INTEGRATION_OUTBOX_DRAIN_LEASES_MS) + 5 min` — so a job still inside its lease is a job, not an exception | **None — the section is read-only on purpose, and the reason is not caution.** Elapsed time cannot tell a dead holder from a paused one, and a compare-and-set cannot unsend a push or un-queue an invoice email the holder already issued. Nor can the row be parked somewhere quiet: **no status in this system is inert**. A dead-lettered row is reset to `PENDING` with `attempts: 0` by WooCommerce's ordinary enqueue path on the next stock change, and by `scheduleXeroAccountingOutbox` on the next Xero sweep — so a one-click "Stop" would have queued the very replay the declaration forbids (Codex o3d-8td2 r5). **And there is no manual exit either — the round-6 copy that named one was itself the defect** (Codex o3d-8td2 r6). Marking a park permanently failed writes the `integration_outbox` row and nothing else: for Xero the owning `AccountingSyncLog` is untouched, `ensureXeroOutboxForPendingSyncLogs` runs first on every sweep, selects it, and `scheduleXeroAccountingOutbox` resets the row to `PENDING` with `attempts: 0` — claimed again on that same sweep. On a row listed here `permanent-fail` and `replay` are one act under two names, and checking the remote system first does not make either safe, because neither is conditioned on what you found. **What to do instead:** read the remote system and believe it over the row (does the Xero invoice exist, did its email go out; what quantity does the product actually have at WooCommerce), correct it **there** by hand — a change made outside the queue cannot be re-queued — and escalate the row id rather than trying to clear it. The guidance the UI shows is structured data (`app/(dashboard)/sync/exceptions/stalled-park-guidance.ts`) so that the prohibition and the recommendation are separate fields a test can tell apart. The withdrawn one-click recovery and the constraint on any future one are `o3d-7qdb` |
