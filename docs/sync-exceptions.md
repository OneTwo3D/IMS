...
| Order reconciliation — drift | latest `ORDER_RECONCILE` `WmsSyncJob`'s discrepancy logs (q66in.4.4, cron `wms-order-reconcile`) | Scheduled IMS-intent-vs-WMS-truth check: `NOT_PUSHED` (eligible order never reached the WMS), `MISSING_IN_WMS` (WMS lost a live order), `ACTIVE_AFTER_CANCEL` (cancelled order still live in the WMS — admins are belled; it may ship) | `MISSING_IN_WMS`: Re-push (link reset to `PENDING_CREATE` for the next push sweep). Others: fix at the linked order / in the WMS |
