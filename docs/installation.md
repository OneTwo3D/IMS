...
**One writer at a time.** The managed crontab block has two writers — the application (Settings → System → Scheduler, and every settings save that affects a schedule) and this installer — and both take the same host-local `flock` on `${DATA_DIR}/.crontab-reconcile.lock` before touching it. The application holds it across the whole read-modify-write (read the `cron_*` settings → read the crontab → write it) and hands the lock descriptor to the `crontab -` child, so the exclusion covers the write even if the app process dies mid-rewrite; the installer takes it with `exec 9>>` + `flock` around its own pipeline.

**That path is the service's systemd `StateDirectory`, and neither writer has one of its own.** The unit the installer writes declares `StateDirectory=one-two-inventory`, systemd creates `/var/lib/one-two-inventory` owned by `imsapp` and exports its absolute path to the service as `$STATE_DIRECTORY`, and the application joins `.crontab-reconcile.lock` onto that. The installer locks `${DATA_DIR}`, which is the same `/var/lib/one-two-inventory`. Do not put the lock beside the application: a hardened unit (`deploy/systemd/ims-stage.service`, `ProtectSystem=strict`) makes the app directory read-only, so a lock file there cannot even be created — a `StateDirectory` is implicitly read-write, which is why it is the one path that works under every supported unit. If you relocate the state directory, change `StateDirectory=` in the unit and re-run the installer; both writers follow it. `OTI_CRONTAB_LOCK_PATH` is a **test-only** override — `$STATE_DIRECTORY` outranks it and production refuses it outright (with a warning on stderr), because a path the application honours and the installer does not is two locks pretending to be one. The queue wait (default 20s) is tunable with `OTI_CRONTAB_LOCK_WAIT_MS`.

Never delete and recreate the lock file — the lock lives on the inode, so a replaced file silently gives two writers two different locks; the installer `touch`es it for that reason. To see who holds it: `fuser -v ${DATA_DIR}/.crontab-reconcile.lock`. A save that cannot get the lock within the wait reports "the scheduler may be behind" and changes nothing; re-apply from Settings → System → Scheduler. **If the lock file cannot be opened at all** — no `StateDirectory=` in the unit, a mis-owned or read-only directory — the reconciliation *refuses*: it does not read or write the crontab, and the error names the path it tried and the `StateDirectory` directive to fix. It never falls back to reconciling without the lock, because an unserialised reconciliation is exactly the defect the lock exists to prevent.
...
| `STATE_DIRECTORY` | Set by systemd from the unit's `StateDirectory=`, not by you. The crontab reconciliation lock is `$STATE_DIRECTORY/.crontab-reconcile.lock`, which is how the application and `scripts/install.sh` land on one file. Absent (running outside systemd) the lock falls back to the working directory |
