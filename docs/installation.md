...
**The service user must be in the `crontab` group, or the application cannot touch the crontab at all.** `/usr/bin/crontab` is setgid (`-rwxr-sr-x root crontab`) and the spool directory is `drwx-wx--T root crontab`, so the binary reaches the spool only through the group its setgid bit confers. A hardened unit sets `NoNewPrivileges=true`, and that makes the kernel **ignore setuid/setgid bits on exec** — so `crontab` runs as the plain service user and fails with `Permission denied` on `crontab -l` *and* `crontab -`. `deploy/systemd/ims-stage.service` therefore carries `SupplementaryGroups=crontab` and (because `ProtectSystem=strict` also makes `/var` read-only) `ReadWritePaths=/var/spool/cron/crontabs`. `NoNewPrivileges=true` stays: a supplementary group is granted by systemd before the exec, not gained by it. Without those two lines the failure is loud rather than silent — the read fails closed, the crontab is left untouched, and the save reports that the scheduler is behind — but the Scheduler is unusable. If you run the application under a unit of your own, either grant the group or accept that the crontab is owned solely by `scripts/install.sh`.

**A write that exits 0 is confirmed by reading it back.** `crontab -` exiting 0 says the command was issued, not that the schedule exists — and on a sandboxed unit those come apart. After a successful write the application re-reads the crontab **while still holding the lock** and compares the managed block against what it wrote. A block that does not match, and a read-back that does not resolve at all, are both reported as a failed reconciliation: the write is not finished until it is confirmed, so "unconfirmed" and "wrong" get the same answer. The recovery — Settings → System → Scheduler → Save & Apply — is idempotent, so the cost of being told this over a crontab that was in fact correct is one repeated save.
