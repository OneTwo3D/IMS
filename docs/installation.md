...
**One writer at a time.** The managed crontab block has two writers — the application (Settings → System → Scheduler, and every settings save that affects a schedule) and this installer — and both take the same host-local `flock` on `${DATA_DIR}/locks/.crontab-reconcile.lock` before touching it. The application holds it across the whole read-modify-write (read the `cron_*` settings → read the crontab → write it) and hands the lock descriptor to the `crontab -` child, so the exclusion covers the write even if the app process dies mid-rewrite; the shell side takes it through `with_crontab_lock` in `scripts/lib/crontab-lock.sh`, which locks a descriptor `prepare_crontab_lock` opened once, early — read-only, at the moment it had just proved the inode — and releases it with `flock --unlock` at the end of each read-modify-write. The descriptor outlives the critical section; the *exclusion* does not, so the application can still reconcile between a cutover's two crontab writes. `prepare_crontab_lock` **stands inside** `${DATA_DIR}/locks` while it creates, checks and opens the lock file, and names the file by its bare filename from there: `${DATA_DIR}` belongs to `imsapp`, so that account can rename the root-owned `locks` directory aside at any moment, and a second resolution of the pathname would then reach whatever it put at the name — including a named pipe, on which a read-only `open` blocks for ever with the service already stopped and the database already fenced.
