...
**An upgrade restarts the application before it touches the crontab.** That flock only excludes a process locking the *same* file, which is true only of a process running the build the installer just deployed, under the unit it just wrote. So section 12 `enable`s the unit and then explicitly `systemctl restart`s it: `enable --now` would not do — its `--now` is a `start`, and `start` on an already-running unit is a no-op, so an upgrade left the previous process alive on the previous bundle and the previous lock path (or none), and the installer then locked the new one beside it. `daemon-reload` alone does not help either; it only changes what the *next* start reads. **If that restart fails, or the unit is not `active` afterwards, the install aborts** — nothing after section 12 runs (no nginx site, no hardening, no logrotate, no cron block), because taking a lock that excludes nothing is worse than not writing the crontab at all. Everything before that point is idempotent, so fix the service (`systemctl status`, `journalctl -u`) and re-run the installer. A hand-deployed upgrade must do the same thing: rebuild, then `systemctl restart`, and only then touch the crontab.
