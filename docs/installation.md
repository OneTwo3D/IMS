...
**One writer at a time.** The managed crontab block has two writers — the application (Settings → System → Scheduler, and every settings save that affects a schedule) and this installer — and both take the same host-local `flock` on `${APP_DIR}/.crontab-reconcile.lock` before touching it. The application holds it across the whole read-modify-write (read the `cron_*` settings → read the crontab → write it) and hands the lock descriptor to the `crontab -` child, so the exclusion covers the write even if the app process dies mid-rewrite; the installer takes it with `exec 9>>` + `flock` around its own pipeline. Override the path with `OTI_CRONTAB_LOCK_PATH` and the queue wait (default 20s) with `OTI_CRONTAB_LOCK_WAIT_MS` only if you have moved the application directory. Never delete and recreate the lock file — the lock lives on the inode, so a replaced file silently gives two writers two different locks; the installer `touch`es it for that reason. To see who holds it: `fuser -v ${APP_DIR}/.crontab-reconcile.lock`. A save that cannot get the lock within the wait reports "the scheduler may be behind" and changes nothing; re-apply from Settings → System → Scheduler.
