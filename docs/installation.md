...
deployment is refused at the **first session-lock acquisition**, whichever of these reaches one first:
a **money post**, the **Xero daily accounting batch**, the **QuickBooks daily accounting batch**, a
**Xero payment write**, a **WMS dispatch sweep** or a **restore**. (Those are six jobs on four locks:
the money post and both daily batches run on `lib/db/pinned-advisory-lock.ts`; the other three hold
their own.) It fails **closed and loud**, naming `DATABASE_SESSION_LOCK_URL` as the thing an operator
can change, rather than quietly paying a document twice.
...
the refusal only at the first session-lock acquisition — and since two of those six jobs are the
**scheduled** daily accounting batches, the first refusal may well be a cron run nobody was watching
rather than an operator's money post. What the check detects is **interposition**,
