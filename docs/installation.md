...
- **orders imported before that rule existed may be carrying a currency nobody stated.** Earlier versions defaulted a missing WooCommerce currency to GBP, and `SalesOrder.currency` also defaults to GBP, so an invented GBP and a genuine one are indistinguishable in the order row — there is no marker separating them, and no query over the orders alone can tell them apart. **No automated separation ships yet**, and none should be trusted until it can prove which delivery created each order: the archived `shopping_webhook_events` payloads are the only positive evidence, they cover webhook-arrived orders only, and a live WooCommerce read is actionable but is not proof of what the order was created with. Correcting such an order is also not a matter of rewriting `currency`: the invented code made the FX lookup return 1:1, so the converted `*Base` totals were computed at par, and a code-only rewrite would replace one inconsistency with another. Any affected order that has already been invoiced, paid or swept into a daily accounting batch must be reconciled by hand. **That evidence has an expiry date, and it is now held open.** The daily retention pass compacts the shopping webhook inbox — it replaces the `payloadJson` of every PROCESSED row past `retention_webhook_events_months` (default **3 months**) with `{}`, keeping the row as an idempotency tombstone — so the archived deliveries this depends on were on a three-month destruction clock. WooCommerce **order** deliveries (`connector = 'woocommerce'`, `resource = 'orders'`) are therefore **exempt from that compaction until `o3d-j7y4` closes**, by a code constant in `lib/connectors/shopping-webhook-retention.ts` rather than a setting, so that turning the destruction back on is a reviewed change. **The exemption is bounded in time, per installation.** Only order deliveries received *before this installation's evidence cutoff* are held; ones received after it are cleared on your schedule like any other, because by then the installation was running the currency guard and no order it imports can be affected. The cutoff marks when *this* installation stopped running the old importer — a run-time fact, different for an install that upgrades today and one that upgrades next month — and it is recorded automatically: the daily cleanup stamps it (as the settings row `legacy_wc_order_evidence_cutoff_at`) the first time it runs on a version carrying the guard, plus a day's margin for a rolling restart to finish draining, insert-only and never moved afterwards. Until an installation has recorded one — a fresh install before its first nightly run, or one whose stored value is unreadable — the hold covers **every** WooCommerce order delivery, which is the safe direction: it over-retains rather than destroying evidence it cannot date. The exemption is a hold on destruction only — it examines no payload and makes no claim about any order — and its cost is that WooCommerce order payloads, which carry billing and delivery names and addresses, are retained past the retention window while it is on. It is shown to the operator on **Settings > System > Data Retention**, beside the window it overrides, with the cutoff instant and the number of deliveries it is currently keeping alive. Two limits are permanent and no later change can repair them: a payload that was **already compacted** cannot be reconstructed from anything else in the database, and a creator-identity column added to the order table later cannot prove, retroactively, what an earlier delivery said. Orders created by the initial import or by the poll/reconcile sweeps never touched the inbox at all and so have no archived delivery to preserve. Tracked as `o3d-j7y4`.
