...
| Every 15 min | `/api/cron/wc-refusal-bell-retry` | Re-sends the admin notification for a WooCommerce order the store completed and IMS refused to fulfil, when the first attempt to write it failed — the retry does not wait for the order to be refused again |
