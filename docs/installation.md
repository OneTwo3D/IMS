...
server session and is freed the moment that session goes back to the pool. Measured rather than
assumed: **PgBouncer 1.24.1** in `pool_mode = transaction` multiplexed **one** client connection across
**two** backends (pids `…721` and `…722`) within a single client connection, and **Odyssey 1.5.3-rc1**
in transaction-pooling mode answered two clients' `pg_try_advisory_lock()` on the same key with `true`
for **both** while `pg_locks` showed neither held it. For the money-post lock that is the same document
paid twice, with no other symptom. Separately, the schema pin travels as a libpq startup
...
deployment is refused at the **first** money post, Xero payment write, WMS dispatch sweep or restore:
it fails **closed and loud**, naming `DATABASE_SESSION_LOCK_URL` as the thing an operator can change,
rather than quietly paying a document twice.
...
the refusal only when the first of those four jobs runs. What the check detects is **interposition**,
...
connection either: the lock still ends with its connection, and the window between the last check and
the remote system accepting the request is closed by connector-side idempotency rather than by any lock
(**o3d-ic9a**).
