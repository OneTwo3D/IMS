...
- **orders imported before that rule existed may be carrying a currency nobody stated.** Earlier versions defaulted a missing WooCommerce currency to GBP, and `SalesOrder.currency` also defaults to GBP, so an invented GBP and a genuine one are indistinguishable in the order row — no query over the orders alone can separate them. Run `npm run wc:currency:audit` (read-only; add `-- --live` to also GET each order from WooCommerce, `-- --limit N` to cap the scan) to collect the evidence that *can* speak: a stored code that is not a canonical three-letter uppercase value, which current code cannot have written; an archived webhook delivery in `shopping_webhook_events` that provably stated no currency, which positively identifies the fallback for orders that arrived by webhook; and a live currency that disagrees with the stored one. The audit reports and exits 1 on findings — it corrects nothing, deliberately: the order's converted `*Base` totals were computed at the invented rate too, so a code-only rewrite would replace one inconsistency with another, and any order already invoiced or paid is flagged `COMMITTED` and must be reconciled by hand.
