...
- **Server Action authorization guards:** `npm run check:server-action-guards` runs both halves — `check:server-action-auth-bypass` (no Server Action may take a serializable option that suppresses its permission check) and `check:server-action-guard-coverage` (every exported async function in a `'use server'` module enforces a guard, delegates to one, or is allowlisted with a reason). They run in `scripts/validate-local.sh` and in their own ungated workflow, `.github/workflows/server-action-auth-guard.yml`. Remember that a `'use server'` export is a public HTTP endpoint: not being imported by any UI, and not being in the facade allowlist, are both irrelevant to who can call it.
