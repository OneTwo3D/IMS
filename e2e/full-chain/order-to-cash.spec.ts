...
test.describe.serial('@full-chain @wc @xero order to cash', () => {
  ...
  test('OC-03: cancel in Woo before shipment -> SO CANCELLED, allocation released, invoice NOT posted', async ({ page }) => {
    ...
    // reservation, and (b) recognise NO revenue — the queued sales invoice must not reach Xero.
    ...
    // Arm sync so the SALES_INVOICE IS queued at import — that is the whole point: prove that draining
    // it after the cancel posts nothing (the o3d-5rs guard skips a CANCELLED order's invoice).
    await setPostingMode({ sync: true, dailyBatch: false })
    ...
    // Drain the pending queue the way an operator would — the import-queued SALES_INVOICE must be
    // skipped for the now-cancelled order, leaving no external document in Xero (o3d-5rs).
    await processPendingXeroSyncViaUi(page)
