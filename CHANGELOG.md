...
- **Landed-cost snapshot refreshes now keep returned quantities and replacement unit costs Decimal-safe until serialization.** Customer and supplier return helpers now return `Prisma.Decimal`, and cost-layer snapshot refreshes accept Decimal inputs before explicitly preserving the existing JSON number shape for `unitCostBase`.
