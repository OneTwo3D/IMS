/**
 * QuickBooksConnector — implements AccountingConnector interface.
 * Main entry point for the QuickBooks Online accounting connector module.
 */
...
export class QuickBooksConnector implements AccountingConnector {
  ...
  async postJournalEntry(entry: JournalEntry): Promise<SyncResult> {
    const result = await pushJournalEntry(entry)
  ...
  async postCreditNote(data: CreditNoteData): Promise<SyncResult> {
    const result = await pushCreditMemo(data)
...
export { getAuthorizationUrl, consumeQuickBooksOAuthState, exchangeCodeForTokens, disconnect, isConnected, getAccessToken } from './auth'
export { syncChartOfAccounts, getQuickBooksTaxCodes } from './accounts'
...
export { processPendingQuickBooksSync, repairQuickBooksBackReferences } from './sync-processor'
export { pollQuickBooksPayments } from './payment-poller'
export { downloadQuickBooksInvoicePdf, saveInvoicePdf } from './invoice-pdf'
export { runDailyBatchSync } from './daily-sync'
