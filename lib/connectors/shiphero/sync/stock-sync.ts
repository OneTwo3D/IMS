...
export async function runShipheroStockSyncForBinding(bindingId: string): Promise<ShipheroStockSyncResult> {
  ...
  try {
    ...
    // staleSyncAlertedAt re-arms the watchdog's stale-sync alert (q66in.4.6).
    await db.externalWmsBinding.update({ where: { id: binding.id }, data: { lastStockSyncAt: now, lastStockSyncStatus: 'SUCCEEDED', staleSyncAlertedAt: null } })
