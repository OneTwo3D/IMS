 ...
 *
 * WHAT IT COSTS AND WHEN IT RUNS. ONCE per process, per `(DATABASE_URL, override)` pair, on the
 * first session-lock connection that gets as far as needing it -- the affinity leg above runs
 * first, so a deployment that is refused for interposition never pays for this. It is two throwaway
 * connections and four statements. Measured on this host: 1-4ms for the witness leg, 3-13ms for the
 * whole pair including both connects. Nothing is paid per query, per checkout, or per later
 * connection; the memo below is keyed on the URL pair.
 *
 * WHY ONCE IS ENOUGH. The property is one of the two ENDPOINTS, not of any one socket: both URLs
 * are read once from the environment by `lib/db/session-lock-pool.ts` and do not change while the
 * process runs, and whether an advisory lock taken through one is visible through the other is
 * settled by which server and database each reaches. That is the opposite of the affinity leg,
 * which is about THIS socket and therefore has to run per connection. The residue is stated rather
 * than hidden: a mid-life failover or DNS change that re-points one endpoint at a different server
 * is not caught, and re-running per connection would not catch it either -- the data path's own
 * connections move with it, and there is always a window between any check and the lock it
 * licenses. What is closed is the operator pointing the override at the wrong instance, which is
 * the finding.
 *
 ...
 * `establishStartupOptionByteSafety()`. It is filed as o3d-6t4q rather than half built here.
 * -------------------------------------------------------------------------------------------- */
 ...
 *
 * @returns {Map<string, Promise<void>>}
 */
...
function probeConnectionConfig(url) {
  const config = { ...pgConnectionConfig(url), connectionTimeoutMillis: SHARED_LOCK_SPACE_PROBE_TIMEOUT_MS }
 ...
 */
async function measureSharedAdvisoryLockSpace(databaseUrl, overrideUrl, createClient) {
  const connect =
    createClient ??
    (async (config) => {
      const pg = await import('pg')
...
/**
 * The same measurement, made ONCE per process per URL pair.
 *
 * The PROMISE is memoised rather than its result, so connections opening concurrently share one
 * probe instead of racing several. A rejection is memoised too, deliberately: a refusal that
 * re-probed on every connection would turn a misconfiguration into a connection storm, and the
 * environment it is about cannot change without the process restarting.
 *
 ...
 */
function sharedAdvisoryLockSpaceEstablished(databaseUrl, overrideUrl, createClient) {
  const memo = sharedLockSpaceMemo()
  const slot = `${databaseUrl} ${overrideUrl}`
  const held = memo.get(slot)
  if (held !== undefined) return held
  const running = measureSharedAdvisoryLockSpace(databaseUrl, overrideUrl, createClient)
  memo.set(slot, running)
  return running
}
 ...
 * @param {{ createClient?: (config: object) => Promise<{ connect(): Promise<unknown>, query(text: string): Promise<{ rows: Array<Record<string, unknown>> }>, end?: () => Promise<unknown> }> }} [options] a stand-in connector, for tests
 * @returns {{ connectionString: string, options?: string, onConnect: (client: { query(text: string): Promise<{ rows: Array<Record<string, unknown>> }> }) => Promise<void> }}
...
export function pgSessionLockConnectionConfig(databaseUrl, sessionLockUrl, purpose = 'a session advisory lock', options = {}) {
  ...
  const sharedLockSpace =
    override === null
      ? null
      : () => sharedAdvisoryLockSpaceEstablished(String(databaseUrl ?? ''), override, options.createClient)
