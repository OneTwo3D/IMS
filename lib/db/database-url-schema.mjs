...
function interposedConnectionRefusal(client, row) {
  ...
  const ours = ownSocketEndpoint(client)
  if (ours === null) {
    return (
...
export async function establishStartupOptionByteSafety(databaseUrl, options = {}) {
  ...
  const ask = async (config, queries) => {
    ...
    try {
      ...
      // The socket is read while the client is still up, because the refusal below is about THIS
      // connection and `end()` in the finally would take the answer away with it.
  ...
  try {
    const { answers: [answer, servedBy], client: measuredOn } = await ask(
    ...
    const interposed = interposedConnectionRefusal(measuredOn, servedBy)
    if (interposed !== null) {
      return settle({
        serverEncoding,
        lcCtype,
