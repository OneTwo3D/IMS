 ...
 */
function sessionLockAffinityGuard(startupOptionGuard, purpose, sharedLockSpace = null) {
  return async (client) => {
    const { rows } = await client.query(SESSION_LOCK_PEER_SQL)
...
/**
 * Tear a probe connection down NOW, rather than asking the server's permission to (o3d-2k5r r27).
 *
 ...
 * @returns {void}
 */
function destroyProbeClient(client) {
 ...
 * options. On expiry BOTH clients are destroyed at the socket (see `destroyProbeClient()`, and
...
async function measureSharedAdvisoryLockSpace(databaseUrl, overrideUrl, createClient, deadlineMs) {
  ...
  const deadline = new Promise((_resolve, reject) => {
    timer = setTimeout(() => {
      expired = true
      for (const client of opened) destroyProbeClient(client)
      opened.clear()
      reject(
    ...
    return await /** @type {Promise<void>} */ (
      Promise.race([
        runSharedAdvisoryLockSpaceProbe(databaseUrl, overrideUrl, createClient, (client) => {
          ...
          if (expired) destroyProbeClient(client)
 ...
 * @param {{ createClient?: (config: object) => Promise<{ connect(): Promise<unknown>, query(text: string): Promise<{ rows: Array<Record<string, unknown>> }>, end?: () => Promise<unknown> }>, probeDeadlineMs?: number }} [options] a stand-in connector and a shortened probe deadline, for tests
 * @returns {{ connectionString: string, options?: string, onConnect: (client: { query(text: string): Promise<{ rows: Array<Record<string, unknown>> }> }) => Promise<void> }}
...
export function pgSessionLockConnectionConfig(databaseUrl, sessionLockUrl, purpose = 'a session advisory lock', options = {}) {
  ...
  return { ...config, onConnect: sessionLockAffinityGuard(config.onConnect, purpose, sharedLockSpace) }
}
