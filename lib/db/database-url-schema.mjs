 ...
 *     round-robin over two backends can put all three on one. This is stated here because the
 *     alternative — refusing every endpoint that cannot be PROVEN single-backend — is a refusal
 *     nobody can ever lift, and it strands every deployment behind a pooler.
 *   • It does NOT SURVIVE A FAILOVER after the fact. Nothing reachable from a synchronous
 *     `pgConnectionConfig()` can: the pool has not connected yet, and the backend a future
 *     connection lands on is not knowable here. What bounds that is the SHAPE OF THE FAILURE —
 *     see below — and the fact that the probe re-runs at every boot (`instrumentation.ts`), in
 *     `preflight:production`, and in `scripts/check-wms-push-state-enum.mjs` before a deploy stops
 *     the old server, so the window is one process lifetime rather than one deployment's.
 *
 * THE SHAPE OF THE FAILURE IS WHY THAT RESIDUE IS ACCEPTABLE. A backend that SPLITS on the byte
 ...
 * startup and hands back something OTHER than what was written — and it is exactly the class the
 * measurement below settles as `carries: false`. Binding the verdict to a backend identity is what
 * stops a `carries: true` earned on one server being spent on another; a re-probe at the next boot
 * is what catches the server having been replaced underneath it.
 * -------------------------------------------------------------------------------------------- */
 ...
 */
const BACKEND_IDENTITY_SQL =
  'select pg_encoding_to_char(encoding) as server_encoding, datctype as lc_ctype, ' +
...
export async function establishStartupOptionByteSafety(databaseUrl, options = {}) {
  ...
  const identities = []
  try {
    const [row, second] = await ask({ connectionString }, [BACKEND_IDENTITY_SQL, BACKEND_IDENTITY_SQL])
  ...
  try {
    const [row] = await ask({ connectionString }, [BACKEND_IDENTITY_SQL])
    identities.push(backendIdentityOf(row))
  } catch (error) {
    return settle({
  ...
  try {
    const [answer, servedBy] = await ask(
...
function nonAsciiOptionByteIsCarried(character, databaseUrl) {
  ...
  // The URL is all this synchronous function can compare against, and it is deliberately NOT
  // treated as if it named a server: the check above says "the same endpoint the probe measured",
  // and it is `establishStartupOptionByteSafety()` that decided whether that endpoint answered as
  // ONE backend before it was allowed to settle a positive verdict at all. `verdict.backend` names
  // which one. See the block comment above for what that does and does not establish.
  return true
}
...
function nonAsciiUpgradePath() {
  ...
  return (
    ...
    'A positive answer is bound to the BACKEND that gave it: the probe requires every connection it ' +
    'opens to this endpoint to be served by the same PostgreSQL before it will grant anything, ' +
    'because a pooler, a load balancer or a failover pair can serve the probe from one server and ' +
    'the application pool from another. ' +
