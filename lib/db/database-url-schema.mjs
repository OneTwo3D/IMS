      ...
      reject(
        new DatabaseUrlSchemaConflictError(
          `This connection is refused because it is about to take a SESSION advisory lock (${purpose}) and ` +
            ...
            `answering, so its socket was DESTROYED rather than left pending: an acquisition that hangs is a ` +
            `money post, an accounting batch, a WMS dispatch sweep or a restore hanging with it, and a server ` +
      ...
      if (interposed !== null) {
        throw new DatabaseUrlSchemaConflictError(
          `This connection is refused because it is about to take a SESSION advisory lock (${purpose}) and it ` +
            ...
            `lives on ONE backend and is freed the moment that backend is handed to someone else, so behind a ` +
            `transaction- or statement-pooling proxy two holders can each be told they hold it and neither ` +
            `does (o3d-2k5r r25 / o3d-a5zz). ${sessionLockDirectRoute()}`,
        )
      }
