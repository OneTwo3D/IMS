 ...
 * a live retargeting: `options=-c%20search_path%3D%5C%C2%A0tenant` is accepted by
 * `splitLibpqOptions()` precisely BECAUSE the U+00A0 is escaped — the escape is how an operator
 * says "mean this character literally" — `unescapeLibpq()` hands back `<U+00A0>tenant`, and
 * `trim()` then silently reduced it to `tenant`. The name pinned into the emitted `options`,
 * compared against `?schema=`, and reported to all three raw gates was thus NOT the name the
 * server resolves; it also made `?schema=tenant` compare EQUAL to an options value naming
 * `<U+00A0>tenant`, so the two halves agreed on paper while resolving different schemas. The same
 * silent cross-schema retargeting rounds 10-14 closed for quotedness, repetition, tab separators,
 * emitted separators and the GUC name, reached this time through the value's own two edges.
 *
 * @param {string} value
 * @returns {string}
 */
 ...
 *   server will, or on a terminal escape that opens a token and closes nothing.
 */
export function splitLibpqOptions(options) {
  ...
  // database's encoding or locale (`NON_ASCII_WHITESPACE`), one the driver and this reader resolve
...
function singleSchemaOfSearchPath(value) {
  ...
  // says "name one schema", advice for a value that already names exactly one. What is actually
  // wrong with it is that it is an UNQUOTED name this module cannot fold the way the server will,
  // which `foldUnquotedIdentifier()` below already refuses in its own words: "Quote it to name it
  // exactly." Restricting the test to the server's own whitespace routes the value to that
  // refusal. Both fail closed; only one tells the operator what to do about it.
