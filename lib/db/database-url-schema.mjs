 ...
 *
 * PULLED OUT OF `BACKEND_IDENTITY_COLUMNS` in r25 because a SECOND question now asks it — whether a
 * connection that is about to take a SESSION advisory lock reaches the backend directly — and that
 * question is asked on connections whose startup options are pure ASCII, where no backend-identity
 * verdict exists to compare against and the identity columns would be dead weight. ONE spelling for
 * both: two hand-written peer column lists are two chances to drift into a comparison that always
 * passes, which is the failure mode this whole module is written against.
 */
const CLIENT_PEER_COLUMNS =
 ...
 *     cannot multiplex. That is a false positive on a working configuration, and it is kept after
 *     re-examination in r24 because it is not a measurement problem that a better probe would
 *     solve: a masquerading NAT and a protocol-terminating proxy present the SAME evidence to both
 *     ends — a peer that is not ours — and what separates them (whether the TCP connection is
 *     end-to-end) is not attestable from SQL or from Node. The choice is therefore refuse-both or
 *     admit-both, not "detect it properly". Refuse: it can only ever affect a NON-ASCII schema (the
 *     ASCII path attaches no check at all), the failure is loud with the two documented ways out,
 *     and the alternative — trusting a connect-time reading of a socket that may be re-pointed —
 *     is a silent cross-schema write.
 *   • Over a UNIX-DOMAIN socket the backend reports no peer at all (`client_addr` NULL,
 *     `client_port` -1), so a pooler and a direct connection are indistinguishable here — and as
 *     of r24 that is REFUSED, not skipped. Round 23 skipped it and recorded the residue as "a
 *     hypothetical proxy that forwards startup options faithfully AND multiplexes AND is reached
 *     over a Unix socket". It is not hypothetical: ODYSSEY does transaction pooling, listens on
 *     Unix sockets, and re-applies the client's startup parameters to each backend it attaches
 *     (`maintain_params`, on by default — it prints so in its own startup rules dump).
 *     And the `search_path` comparison named as the cover does not cover it: it runs ONCE, in
 *     `onConnect`, so it says what the session held at connect time and nothing about the backend
 *     that runs the next statement. One reading on a multiplexed session is not a property of the
 *     session. So an absent peer is an UNPROVEN comparison, and an unproven comparison is a
 *     refusal.
 *
 *     WHAT THAT COSTS, EXACTLY: a deployment that connects over a Unix-domain socket AND names a
 *     schema whose characters need a non-ASCII startup option is refused at boot. Nothing else
 *     changes — an ASCII schema attaches no guard at all, and a Unix socket with an ASCII schema
 *     is untouched. There are TWO ways out and both are cheap: point `DATABASE_URL` at
 *     `127.0.0.1`/`localhost` over TCP, where the peer comparison can be made and a direct
 *     connection passes it (the live tests below do exactly this), or rename the schema to ASCII.
 *
 *     ODYSSEY, MEASURED RATHER THAN CITED (o3d-2k5r r24). Built from source at 1.5.3-rc1 and put
 *     in front of the local PostgreSQL 17 with a Unix socket on BOTH legs and `pool "transaction"`:
 *
 *       • THE MULTIPLEXING HAZARD IS REAL AND IT IS NOT ABOUT THE SCHEMA. Two logical clients each
 *         took `pg_try_advisory_lock(4242)` and BOTH got `true`; the first client's own
 *         `pg_locks` showed ZERO advisory locks the statement after it "acquired" one. The session
 *         exclusion the money-post, payment-write and dispatch-sweep locks are built on simply
 *         does not exist behind it — the backend goes back to the pool between statements and
 *         `pool_discard` takes the lock with it.
 *       • AND THE EVIDENCE THIS CHECK READS IS IDENTICAL TO A DIRECT CONNECTION'S. The backend
 *         reported `client_addr` NULL and `client_port` -1 through Odyssey exactly as it does
 *         direct, and node reported no `localPort` on either. r23 could not have told them apart,
 *         which is the finding.
 *       • WHAT ODYSSEY DOES TO THE PIN, THOUGH, IS NOT WHAT WAS INFERRED. It does not forward the
 *         startup `options` faithfully: `current_setting('search_path')` came back
 *         `"""ten ant"""` — re-quoted — and `current_schema()` was NULL, for an ASCII schema
 *         as well as a non-ASCII one, so the pin resolves nothing at all. The probe's own custom
 *         GUC was dropped outright (`unrecognized configuration parameter`). So for THIS pooler the
 *         r23 `search_path` leg would in fact have refused, and Codex's "if Odyssey carries the
 *         startup settings" is a conditional Odyssey does not satisfy. That is recorded because it
 *         is what was measured — and it does not save r23: the leg that would have caught it is a
 *         per-pooler quirk read once at connect time, while the peer evidence is identical for
 *         every proxy on this path, faithful or not.
 *
 *     WHAT THIS DOES NOT CLOSE. An ASCII schema attaches NO guard, so an ASCII deployment behind a
 *     transaction pooler still gets the lock result above with nothing detecting it. That is a
 *     property of the codebase this branch did not introduce and does not fix; it is filed as
 *     o3d-a5zz, with the reproduction and three candidate shapes, rather than hidden.
 *
 *     CAN UNIX-SESSION AFFINITY BE PROVEN INSTEAD? In principle yes, and not by another inference:
 *     `SO_PEERCRED` on the client socket yields the PID of the process on the other end, and
 *     comparing it against `pg_backend_pid()` is the exact Unix analogue of the address:port
 *     comparison — same shape, same strength. It is not shipped because it is not reachable from
 *     here: Node exposes no `SO_PEERCRED` (it would need a native addon in the boot path), and the
 *     `/proc/net/unix` route resolves only to a socket inode, which turns into a PID only by
 *     scanning `/proc/<pid>/fd` of a process running as a DIFFERENT user — unreadable unless the
 *     application runs as root or as `postgres`. A native dependency plus elevated privilege, to
 *     avoid a one-line change to the connection string, is not a trade worth making.
 *
 * WHY ONE CONNECT-TIME READING IS ENOUGH — BUT ONLY ONCE THIS PASSES (o3d-2k5r r24). Codex's
 * objection to r23 was that "the subsequent search_path assertion runs only once in onConnect, so
 * it proves neither that later statements retain" the pin. That is exactly right where the peer is
 * absent or wrong, and it stops being true where the peer IS ours: a TCP connection does not
 * migrate, so if the backend that answered names the far end of the socket this process is holding,
 * then every later statement on that socket reaches THAT backend, and a reading taken once is a
 * property of the connection rather than a sample of it. The identity and search_path legs are
 * therefore not three independent samples — they are one reading made load-bearing by the peer
 * check that runs beside them, which is why the peer check may not be skipped for any reason.
 *
 ...
 *
 * The row is whatever `BACKEND_IDENTITY_COLUMNS` produced. Directness has to be SHOWN: `null` is
 * returned only when the backend named a peer and that peer is this process's own socket. An
 * absent peer — a Unix-domain socket (`client_port` -1), or an answer that did not carry the
 * columns at all — is a refusal, because unanswerable is not the same as answered well; see the
 * block comment above for what that costs and what the ways out are.
 *
 * @param {unknown} client
 * @param {Record<string, unknown>} row
 * @returns {string | null}
...
function interposedPeerRefusal(ours, row) {
  ...
  // THE BACKEND NAMED NO PEER (o3d-2k5r r24, Codex HIGH). `client_port` is -1 over a Unix-domain
  // socket, and empty when the answer did not carry the columns at all. Round 23 read that as
  // "unanswerable, so skip"; it is unanswerable, and that is precisely why it cannot pass. See
  // the block comment above: Odyssey pools transactions over Unix sockets while maintaining the
  // client's startup parameters, so on this path a multiplexing proxy and a direct connection
  // produce identical evidence, and the `search_path` leg that was named as the cover is a single
  // connect-time reading that says nothing about the next statement.
  if (backendSees.port === '' || backendSees.port === '-1') {
    return (
      `The backend reports no client peer for this connection (client_addr NULL, client_port ` +
      `${JSON.stringify(backendSees.port === '' ? '' : backendSees.port)}), which is what a UNIX-DOMAIN socket ` +
      `looks like — and it is also what a transaction-pooling proxy on a Unix socket looks like, byte for ` +
      `byte. Odyssey pools transactions over Unix sockets and maintains the client's startup parameters ` +
      `across backend attachments, so on this path there is no reading that separates the two, and the ` +
      `search_path this connection reports is one connect-time sample that says nothing about the backend ` +
      `that runs the next statement. Directness cannot be shown here, so the non-ASCII startup option is ` +
      `refused rather than trusted. Connect over TCP to the same server (host=127.0.0.1), where the ` +
      `backend names the peer and a direct connection is admitted, or rename the schema to ASCII.`
    )
  }
...
export function nonAsciiStartupOptionCharacters(databaseUrl) {
  ...
  // Joins the SQLSTATE characters of a parse failure — single characters of a Postgres error code.
...
export async function establishStartupOptionByteSafety(databaseUrl, options = {}) {
  ...
  try {
    ...
    // The WMS boundary guard's constant fold cannot evaluate `.split`/`.pop`, and an unevaluable
    // constant is reported rather than read as empty. The constant is `ims.startup_option_probe`
    // and this takes its second segment, so it cannot spell a WMS connector id.
    // wms-connector-boundary-ok: o3d-lhjh: constant GUC name split on '.', never a connector id
 ...
 * put `current_schema()` on `tenant` for all six ASCII whitespace characters — and on
 * `<character>tenant`, the character KEPT as part of the identifier, for U+00A0, U+2007, U+2028 and
 * U+FEFF.
 *
 * THE LIST IS A PROPERTY OF THE SERVER'S MAJOR VERSION, AND THIS SET IS THE PART EVERY MAJOR AGREES
 * ON (o3d-ik1j). PostgreSQL commit ae6d06f09684d8f8a7084514c9b35a274babca61 ("Handle \v as a
 * whitespace character in parsers", 2023-07-05) added the VERTICAL TAB to `scanner_isspace()` in
 * PostgreSQL 17 and was deliberately not back-patched. `src/backend/parser/scansup.c` on
 * REL_14_STABLE and REL_16_STABLE lists `' '`, `'\t'`, `'\n'`, `'\r'`, `'\f'`; on REL_17_STABLE it
 * lists those and `'\v'` (read 2026-09-19). Measured on real servers: `set_config('search_path',
 * chr(11) || 'tenant')` lands on the seven-character `<VT>tenant` on 16.14 and on `tenant` on 17.11.
 * This set used to hold all six — 17's list — so on 16 and below the module trimmed a character the
 * server KEEPS, pinned `tenant` while the URL named `<VT>tenant`, and reported `?schema=tenant` as
 * agreeing with it: a silent cross-schema retargeting and a disarmed conflict gate, on the version
 * this repository's own CI runs.
 *
 * WHY THE INTERSECTION AND A REFUSAL, NOT A VERSION-AWARE SET. Every caller of this reader —
 * `resolveDatabaseUrlSchema()`, `pgConnectionConfig()`, `prismaAdapterSchemaOptions()` — is
 * synchronous and runs when a pool or adapter is CONFIGURED, before any connection exists, so there
 * is no server to ask at the moment the answer is needed. Borrowing a version from an earlier probe
 * would make the pin depend on whether, and against which server, a probe happened to run — and a
 * failover or an in-place major upgrade changes the answer under a URL that did not change, which is
 * the very drift this module refuses elsewhere. So only the five characters trimmed on EVERY major
 * are trimmed here, and a vertical tab where the server would have to decide (outside a quoted
 * element) is REFUSED by `searchPathSchemaOf()` on every version — see `VERSION_DEPENDENT_WHITESPACE`.
 * Inside a quoted element it is literal on every version and is carried unchanged.
 *
 * `LIBPQ_OPTION_SEPARATORS` is a different function's list (`pg_split_opts()`, libc `isspace()`),
 * which has held the vertical tab on every version; the two are deliberately not aliased.
 */
const SCANNER_WHITESPACE = new Set([' ', '\t', '\n', '\f', '\r'])

/**
 * The characters `scanner_isspace()` answers DIFFERENTLY for across the PostgreSQL majors this
 * module can be pointed at (o3d-ik1j): whitespace from 17, part of an identifier before it. Where
 * one of these stands outside a quoted `search_path` element, which schema the URL names depends
 * on the server's major, so it is refused rather than guessed at.
 */
const VERSION_DEPENDENT_WHITESPACE = new Set(['\v'])
 ...
 * locale) silently re-opens a retargeting it never knew had been closed. The five ASCII characters
 * it strips are still very much reachable, and `tests/db/connection-schema-pinning.test.ts` still
 * fails if they stop being stripped. The sixth, the vertical tab, is not stripped: see
 * `SCANNER_WHITESPACE` (o3d-ik1j).
 *
 * @param {string} value
 * @returns {string}
 */
...
/**
 * The name of the first version-dependent whitespace character standing OUTSIDE double quotes in a
 * `search_path` value, or `null` when there is none (o3d-ik1j).
 *
 * The quote tracking is `SplitIdentifierString()`'s: a `"` opens a quoted element and the next `"`
 * closes it, a doubled `""` inside being a closing quote immediately reopened, which toggles twice
 * and so leaves the state where it was — exactly as the doubled quote stays inside the name.
 *
 * @param {string} value
 * @returns {string | null}
 */
function versionDependentWhitespaceOutsideQuotes(value) {
  let quoted = false
  for (const character of value) {
    if (character === '"') quoted = !quoted
    else if (!quoted && VERSION_DEPENDENT_WHITESPACE.has(character)) {
      return character === '\v' ? 'vertical tab (\\v, 0x0B)' : `character U+${character.codePointAt(0).toString(16).toUpperCase().padStart(4, '0')}`
    }
  }
  return null
}
...
function singleSchemaOfSearchPath(value) {
  ...
  // The same five characters again (the vertical tab never reaches here — o3d-ik1j), and NOT `\s` (o3d-2k5r r15, Codex HIGH). `\s` is Unicode-wide,
  ...
  // rule is still the server's rule about the server's own whitespace, and it is what sends
  // `tenant<TAB>x` to a refusal instead of pinning part of it.
...
function searchPathSchemaOf(url, databaseUrl) {
  ...
  // A VERTICAL TAB THE SERVER WOULD HAVE TO DECIDE ABOUT, REFUSED ON EVERY VERSION (o3d-ik1j). Outside
  // a quoted element it is trimmed (or ends the name) on PostgreSQL 17 and above and is PART OF THE
  // NAME on 16 and below, so `\<VT>tenant` names `tenant` on one server and `<VT>tenant` on another.
  // Refusing it here, before the element is read, is what keeps the pin, the `?schema=` comparison
  // and the raw gates on one answer that holds on every major. Inside double quotes it is literal on
  // every version and reaches the reader unchanged.
  const ambiguous = versionDependentWhitespaceOutsideQuotes(String(value))
  if (ambiguous !== null) {
    throw new DatabaseUrlSchemaConflictError(
      `DATABASE_URL sets options=-c search_path=${JSON.stringify(value)}, which has a ${ambiguous} outside a quoted ` +
        'name. PostgreSQL 17 and above treat that character as whitespace in a search_path element and ' +
        'PostgreSQL 16 and below treat it as part of the name, so the schema this URL names depends on the ' +
        "server's major version — and a pin read from it would be right on one server and a different, " +
        'existing schema on another (o3d-ik1j). Remove the character, or put the name in double quotes if ' +
        'it really is part of the schema name.',
    )
  }
...
/* ----------------------------------------------------------------------------------------------
 * A SESSION ADVISORY LOCK NEEDS THE CONNECTION TO BE THE CONNECTION (o3d-2k5r r25, Codex HIGH;
 * o3d-a5zz).
 *
 * WHAT WAS WRONG. `startupOptionBackendGuard()` above attaches `onConnect` ONLY when the composed
 * startup `options` carries a non-ASCII byte, because that is the only thing a deployment VERDICT
 * is spent on and every extra round trip on the main data pool is paid on every connection. The
 * interposition leg of that guard — "is the backend that answered on the other end of MY socket" —
 * rode along inside it, and so inherited the exemption. An ASCII schema, which is every ordinary
 * deployment, attached NO check at all.
 *
 * That is not a cosmetic gap for one class of caller. MEASURED (o3d-2k5r r24) against Odyssey
 * 1.5.3-rc1 in `pool "transaction"` mode in front of PostgreSQL 17: two logical clients each ran
 * `select pg_try_advisory_lock(4242)` and BOTH got `true`, and the first client's own
 * `select count(*) from pg_locks where locktype = 'advisory'` came back ZERO the statement after it
 * "acquired" one. The backend returns to the pool between statements and takes the session lock with
 * it. Everything this repository builds on a session advisory lock held ACROSS a callback —
 * `withMoneyPostLock` (which decides whether an external payment goes out), the Xero payment-write
 * lock, the WMS dispatch-sweep lock, the restore selection lock — therefore excludes nothing behind
 * such a proxy, silently, on an ASCII schema. `AdvisoryLockLostError` does not see it: the `lost`
 * flag reports a DEAD connection, not a live one whose backend was swapped underneath it.
 *
 * SO THE REQUIREMENT IS ATTACHED TO THE THING THAT TAKES THE LOCK, NOT TO THE CHARACTER SET.
 * `pgSessionLockConnectionConfig()` returns a config whose `onConnect` ALWAYS proves affinity, and
 * `lib/db/session-lock-pool.ts` is the only way the lock holders build a pool or a client — so a
 * new lock cannot be written that forgets to ask, and `tests/db/guarded-pool-routing.test.ts`
 * fails if one is. A caller that can forget is the several-readers-one-fixed shape this branch
 * keeps finding.
 *
 * THE COST IS PAID EXACTLY WHERE IT APPLIES, AND IT IS NOT THE DATA PATH. `pgConnectionConfig()` is
 * UNCHANGED: an ASCII data pool still gets no hook and pays nothing, which is the reason the
 * exemption existed. The advisory-lock pools are `max: 2`-`4` and serve a handful of low-frequency
 * jobs, so one extra round trip per NEW PHYSICAL CONNECTION — never per query, never on a reuse —
 * is negligible where it buys the only thing that makes the exclusion an exclusion.
 *
 * THE AFFINITY LEG IS RUN HERE RATHER THAN LEFT TO THE NON-ASCII GUARD, even though that guard runs
 * it too. Delegating would make the money path's protection a side effect of a check written for a
 * different question: an edit to `startupOptionBackendGuard()` could drop the peer comparison and
 * every lock in the repository would silently stop being exclusive, with no test failing that says
 * so. Two round trips on the non-ASCII lock path is the price of that leg being independently
 * enforced, and the non-ASCII lock path is the rarest connection in the deployment.
 *
 * WHAT THIS DOES NOT CLOSE, said plainly. Proving the connection is not interposed proves that the
 * session that took the lock is the session that keeps it. It does NOT make a session advisory lock
 * a durable exclusion mechanism: the lock still dies with its connection, so a dropped socket, a
 * server restart or a `pg_terminate_backend` frees it while the holder is mid-callback. That is
 * what `lost`/`assertHeld` detect after the fact and what a durable row/lease/fencing shape would
 * remove; it is o3d-ic9a and it stands on its own merits.
 * -------------------------------------------------------------------------------------------- */

/**
 * The environment variable that gives the session-lock connections a route PAST a pooler, so the
 * refusal below names something an operator can actually change.
 */
export const SESSION_LOCK_DATABASE_URL_ENV = 'DATABASE_SESSION_LOCK_URL'

/** Just the peer columns: the ONE question a session-lock connection has to settle. */
const SESSION_LOCK_PEER_SQL = `select ${CLIENT_PEER_COLUMNS}`

/* ----------------------------------------------------------------------------------------------
 * THE DEADLINE BELONGED ON THE CONNECTION THE PROBE EXISTS TO LICENSE (o3d-2k5r r28, Codex MEDIUM).
 *
 * WHAT WAS WRONG, and it is worth naming because it is the shape this branch keeps producing: r27
 * bounded the PROBE and not the thing the probe is about. The whole-probe deadline
 * (`SHARED_LOCK_SPACE_PROBE_DEADLINE_MS`) covers the two THROWAWAY clients the shared-lock-space
 * measurement opens. But those clients are opened from inside `sessionLockAffinityGuard()`, which
 * runs as the lock connection's `onConnect`, and its FIRST statement -- `select client_addr, ...`
 * on the actual lock socket -- had no bound of any kind. A server or pooler that completes the TCP
 * handshake and the startup exchange and then stops answering leaves that query pending for ever:
 * the probe deadline never starts, because the probe is never reached, and no probe client is ever
 * created. Codex reproduced exactly that -- `probeDeadlineMs: 25`, still pending at 100ms, zero
 * probe clients. A protection applied to the sample and not to the sampled is not a protection.
 *
 * SO THE BOUND IS ATTACHED TO THE LOCK CLIENT ITSELF, in two layers, and on expiry BOTH destroy
 * that client's SOCKET rather than calling `end()` -- `end()` writes a Terminate and waits for the
 * server to close, which is precisely what a wedged server will not do (see
 * `destroyClientSocket()`). Destroying the socket is also what makes the pool path work: a
 * `pool.connect()` whose `onConnect` is wedged cannot be cancelled from outside, because the caller
 * has no handle on the client yet -- but the guard runs WITH the client in hand, so killing the
 * socket from the inside is what causes the pending `connect()` to reject and the pool to discard
 * the connection.
 *
 *   - PER STATEMENT: the affinity query gets `SESSION_LOCK_PEER_QUERY_TIMEOUT_MS`. Client-side,
 *     for the reason `SHARED_LOCK_SPACE_QUERY_TIMEOUT_MS` is: a server `statement_timeout` travels
 *     as a startup option and this module has MEASURED PgBouncer accepting the connection and
 *     silently discarding those. It is applied around this ONE query rather than as a
 *     connection-wide `query_timeout`, because the connection is then handed to a lock holder and a
 *     blanket timeout on it would silently bound the holder's own statements too.
 *   - OVER THE WHOLE ACQUISITION: `SESSION_LOCK_ACQUISITION_DEADLINE_MS` wraps every leg of the
 *     guard -- affinity, the startup-option verdict, and the shared-lock-space probe -- so a stall
 *     in a leg that has no bound of its own, or in a stand-in that ignores the ones it has, still
 *     ends. It is the backstop; the per-leg bounds are what make the ordinary stall cheap.
 *
 * WHAT IT DOES NOT CLOSE. A bounded acquisition fails CLOSED: the lock is not taken and the caller
 * is told why. It does not make a session advisory lock a sufficient exclusion for money movement
 * -- see the block over `pgSessionLockConnectionConfig()` and o3d-ic9a.
 * -------------------------------------------------------------------------------------------- */

/**
 * How long the affinity query -- the one statement on the ACTUAL lock socket -- may take.
 *
 * The same 5s as the probe's per-statement bound, and the same reasoning: this is the answer-or-
 * refuse bound for a single round trip on a healthy connection, measured here in single-digit
 * milliseconds.
 */
const SESSION_LOCK_PEER_QUERY_TIMEOUT_MS = 5_000

/**
 * How long the WHOLE of a session-lock acquisition may take: the connect, every leg of the guard,
 * and the shared-lock-space probe inside it.
 *
 * Deliberately LARGER than the sum of the bounds it backs (a 5s affinity query plus the probe's own
 * 20s backstop), so that a leg which does have a bound reaches it first and produces the specific
 * refusal, and this one only ever catches a stall no per-leg bound covers. Exported because
 * `lib/db/session-lock-pool.ts` applies the same number to the part of an acquisition that happens
 * OUTSIDE the guard -- the connect itself, and the per-acquisition re-measurement.
 */
export const SESSION_LOCK_ACQUISITION_DEADLINE_MS = 30_000

/**
 * Run one leg of a lock acquisition under a deadline that, on expiry, DESTROYS THE LOCK CLIENT'S
 * SOCKET and refuses.
 *
 * The timer is not `unref()`'d, for the reason the probe's is not: an unref'd timer does not hold
 * the event loop open, so an acquisition that stalls with nothing else pending would let the
 * process exit before the deadline it is relying on ever fired. It is cleared on every path.
 *
 * @template T
 * @param {unknown} client the ACTUAL lock client, whose socket is destroyed on expiry
 * @param {number} deadlineMs
 * @param {string} what the leg, named in the refusal
 * @param {string} purpose what the lock is for
 * @param {() => Promise<T>} run
 * @returns {Promise<T>}
 */
async function withLockClientDeadline(client, deadlineMs, what, purpose, run) {
  ...
  const deadline = new Promise((_resolve, reject) => {
    timer = setTimeout(() => {
      ...
      reject(
        new DatabaseUrlSchemaConflictError(
          `This connection is refused because it is about to take a SESSION advisory lock (${purpose}) and ` +
            `${what} did not finish within ${deadlineMs}ms. The connection was accepted and then stopped ` +
            `answering, so its socket was DESTROYED rather than left pending: an acquisition that hangs is whatever ` +
            `was taking the lock hanging with it -- a money post, an accounting batch, a WMS dispatch sweep or a ` +
            `restore among them -- and a server ` +
            `that never replies is indistinguishable from one that is about to say "yes". This check may only ` +
            `be passed by an answer. ${sessionLockDirectRoute()}`,
        ),
      )
    }, deadlineMs)
  })
  // The loser of the race still settles; without a handler a rejected deadline would surface as an
  // unhandled rejection after the acquisition has already been admitted.
  deadline.catch(() => undefined)
  const running = run()
  // The abandoned leg still settles -- typically by rejecting, because its socket has just been
  // destroyed underneath it -- and nobody is waiting for it once the deadline has won.
  running.catch(() => undefined)
  try {
    return await /** @type {Promise<T>} */ (Promise.race([running, deadline]))
  } finally {
...
/**
 * What an operator behind a pooler has to CHANGE — appended to every session-lock refusal, because
 * "the connection was refused" is not an instruction.
 *
 * @returns {string}
 */
function sessionLockDirectRoute() {
  return (
    `WHAT TO CHANGE: give the lock connections a route that reaches PostgreSQL DIRECTLY, and leave ` +
    `DATABASE_URL pointing at the pooler for ordinary query traffic. Set ${SESSION_LOCK_DATABASE_URL_ENV} to a ` +
    `non-pooled URL for the SAME database and schema as DATABASE_URL — the PostgreSQL host and port ` +
    `(commonly 5432) rather than the pooler's — and only the advisory-lock pools will use it. It must be TCP: ` +
    `over a Unix-domain socket the backend reports no peer at all (client_addr NULL, client_port -1), so a ` +
    `pooler and a direct connection are indistinguishable there and directness cannot be shown; use ` +
    `host=127.0.0.1. DO NOT route around this by removing the check. Measured against Odyssey 1.5.3-rc1 in ` +
    `transaction-pooling mode, two clients each acquired the same advisory key and each were told they held ` +
    `it while pg_locks showed neither did — which for the money-post lock means the same document can be ` +
    `paid twice.`
  )
}

/**
 * The `onConnect` for a connection that will take a SESSION advisory lock: prove affinity ALWAYS,
 * then run whatever the startup-option verdict also requires.
 *
 * `sharedLockSpace` is the r26 leg, and it runs LAST for a reason: the two legs above are answered
 * by the connection already in hand, while this one opens two more, so a deployment that is refused
 * for interposition or for an unmeasured startup byte never pays for it. It is absent entirely
 * unless `DATABASE_SESSION_LOCK_URL` is set -- see the block comment below for why the question does
 * not arise without one -- and memoised when it is, so it is answered once per process.
 *
 * EVERY LEG RUNS UNDER A DEADLINE THAT DESTROYS THIS CLIENT'S SOCKET (o3d-2k5r r28, Codex MEDIUM).
 * The affinity query is the first statement on the actual lock socket and used to be unbounded, so
 * a server that accepted the connection and then went quiet hung the acquisition for ever without
 * the probe -- and therefore the probe's deadline -- ever being reached. See the block above.
 *
 * @param {((client: { query(text: string): Promise<{ rows: Array<Record<string, unknown>> }> }) => Promise<void>) | undefined} startupOptionGuard
 * @param {string} purpose what the lock is for, so the refusal names it
 * @param {(() => Promise<void>) | null} [sharedLockSpace] the override's shared-lock-space measurement
 * @param {{ peerQueryTimeoutMs?: number, acquisitionDeadlineMs?: number }} [bounds] shortened deadlines, for tests
 * @returns {(client: { query(text: string): Promise<{ rows: Array<Record<string, unknown>> }> }) => Promise<void>}
 */
function sessionLockAffinityGuard(startupOptionGuard, purpose, sharedLockSpace = null, bounds = {}) {
  const peerQueryTimeoutMs = bounds.peerQueryTimeoutMs ?? SESSION_LOCK_PEER_QUERY_TIMEOUT_MS
  const acquisitionDeadlineMs = bounds.acquisitionDeadlineMs ?? SESSION_LOCK_ACQUISITION_DEADLINE_MS
  return async (client) =>
    withLockClientDeadline(client, acquisitionDeadlineMs, 'the session-lock acquisition', purpose, async () => {
      const { rows } = await withLockClientDeadline(
        client,
        peerQueryTimeoutMs,
        'the connection-affinity query',
        purpose,
        () => Promise.resolve(client.query(SESSION_LOCK_PEER_SQL)),
      )
      const interposed = interposedConnectionRefusal(client, rows[0] ?? {})
      if (interposed !== null) {
        throw new DatabaseUrlSchemaConflictError(
          `This connection is refused because it is about to take a SESSION advisory lock (${purpose}) and it ` +
            `cannot be shown to reach the PostgreSQL backend directly. ${interposed} A session advisory lock ` +
            `lives on ONE backend session, and a transaction- or statement-pooling proxy DESTROYS the affinity ` +
            `between the client that took the lock and the backend that holds it. What then becomes of the lock ` +
            `is per implementation, not a property of pooling: the pooler may reset or discard the server ` +
            `connection and RELEASE the lock, or it may leave the lock ATTACHED to a backend it hands to another ` +
            `client -- which then silently excludes that client, or makes it the accidental owner. Measured, and ` +
            `attributed rather than generalised: Odyssey 1.5.3-rc1 in transaction-pooling mode released it ` +
            `immediately, so two holders were each told they held the same key while pg_locks showed neither ` +
            `did (o3d-2k5r r25 / o3d-a5zz). Either outcome breaks the exclusion. ${sessionLockDirectRoute()}`,
        )
      }
      if (startupOptionGuard) await startupOptionGuard(client)
      if (sharedLockSpace) await sharedLockSpace()
    })
}

/**
 * The database a URL names, for comparing an override against the application's own URL.
 *
 * @param {string} url
 * @returns {string | null}
 */
...
/* ----------------------------------------------------------------------------------------------
 * SAME DATABASE AND SCHEMA NAMES DO NOT PROVE THE SAME INSTANCE (o3d-2k5r r26, Codex HIGH).
 *
 * WHAT WAS WRONG. The override validation below compares the decoded database pathname and the
 * resolved schema, and then builds the lock connection entirely from `DATABASE_SESSION_LOCK_URL`.
 * Host and port are deliberately NOT compared, because differing is the whole point of the
 * override -- which left the check with NO instance identity at all.
 * `postgres://pooler/ims?schema=public` and `postgres://some-other-server/ims?schema=public`
 * compare equal on both fields. Every session lock in the process would then be taken on a server
 * the work never touches, every holder would be told it holds it, and the only symptom of
 * `withMoneyPostLock` excluding nobody is the same document being paid twice.
 *
 * WHAT IS CHECKED INSTEAD, AND WHY IT IS NOT AN IDENTIFIER. The lock does not need "the same
 * instance" as a fact about the world; it needs ONE property, and that property is directly
 * measurable: AN ADVISORY LOCK TAKEN ON THE OVERRIDE CONNECTION MUST BE VISIBLE TO THE DATA PATH.
 * So it is taken, and the data path is asked for the same key, and the answer must be "no". Shared
 * lock space is the thing that matters; instance identity is only ever a proxy for it.
 *
 * THE PROXY WAS MEASURED AND IT DOES NOT HOLD, which is why it is not used. On this host
 * (PostgreSQL 17.11, Debian 13):
 *
 *   - `pg_control_system()` IS readable by the unprivileged application role -- `proacl` is NULL,
 *     i.e. the stock EXECUTE-to-PUBLIC -- so the permission objection is not the reason to reject
 *     it. The reason is that it does not answer the question. A `pg_basebackup` clone of the live
 *     cluster reported system identifier 7624286827215704424, BYTE-IDENTICAL to its origin. A
 *     restored clone and a streaming replica are exactly the case Codex named, and a system
 *     identifier ADMITS them.
 *   - A database OID is not unique across clusters either, measured rather than assumed: an
 *     independently `initdb`-ed cluster with a database of the same name reported oid 16385, the
 *     same value the live cluster reports for its own database.
 *
 *   Against that same clone -- identical system identifier, identical database name, identical
 *   schema, identical database OID -- the property test answers correctly: the holder took
 *   `pg_try_advisory_lock`, the witness on the other URL ACQUIRED THE SAME KEY, and the override is
 *   refused. That is the one reading that separates them.
 *
 * WHAT IT COSTS AND WHEN IT RUNS. Per LOCK ACQUISITION (o3d-2k5r r27), not once per process: every
 * checkout from a session-lock pool and every connect of a session-lock client re-measures, unless
 * the connection it just opened ran the probe itself on the way up. The affinity leg above runs
 * first, so a deployment refused for interposition never pays for this. It is two throwaway
 * connections and four statements. Measured on this host: 1-4ms for the witness leg, 3-13ms for the
 * whole pair including both connects. Nothing is paid per query, and nothing at all is paid by the
 * data path or by a deployment that sets no override.
 *
 * WHY PER ACQUISITION RATHER THAN ONCE (Codex HIGH, r27). Until r27 the verdict was memoised for
 * the life of the process, and the argument for that was that both URLs are read once from the
 * environment and do not change. The strings do not change; WHAT THEY REACH DOES. A pooler
 * restarted onto another primary, a DNS record re-pointed, a managed failover promoting a replica
 * -- none of them edits an environment variable, and after any of them a process that measured at
 * boot goes on treating a boot-time sample as authorisation for an EXTERNAL MONEY POST for as long
 * as it lives, while a process started after the change locks the other server. Both are told they
 * hold the lock; the only symptom is the document paid twice. Re-measuring per acquisition costs
 * 3-13ms on jobs that run at most a few times a minute and takes the exposure from "the rest of
 * this process's life" to "the milliseconds between the probe and the lock".
 *
 * WHAT THIS DOES NOT CLAIM, stated here because narrowing a window is easy to misread as closing
 * one. This check DETECTS a configuration in which a session advisory lock demonstrably does not
 * exclude -- an override that reaches a different PostgreSQL from the data path -- and refuses it.
 * It does NOT make a session advisory lock a sufficient exclusion for money movement, and nothing
 * in this branch does. A check is a SAMPLE taken before the thing it licenses; there is always a
 * window between the two, and no number of samples turns into the property. The session lock also
 * remains non-durable (it dies with its connection), remains dependent on the deployment's
 * connection topology, and `lost` remains a best-effort notification. What actually settles the
 * money path is durable, fenced state written through the authoritative `DATABASE_URL` transaction,
 * paired with connector idempotency -- filed as o3d-ic9a (P1 since the 2026-08-28 rescope, which
 * made connector idempotency the first step rather than the lease), and deliberately NOT
 * attempted inside this branch: a money-path exclusion mechanism is not rewritten inside a WMS-enum
 * branch, behind a review cycle about startup-option bytes.
 *
 * IT MUST NOT FALSELY REFUSE THE ONLY DEPLOYMENT IT EXISTS FOR -- the override is set precisely
 * because DATABASE_URL goes through a pooler, so the witness leg has to work through one. Measured
 * against PgBouncer 1.24.1 in `pool_mode = transaction` in front of the same PostgreSQL 17.11:
 * override direct + DATABASE_URL through the pooler ADMITTED (witness blocked, 2ms); the same with
 * the witness carrying the `-c search_path="public"` startup option `pgConnectionConfig()` composes
 * ADMITTED (1ms); an override pointing at the clone while DATABASE_URL goes through the pooler at
 * the origin REFUSED. A plain TCP relay in front of the origin behaves the same way.
 *
 * THE WITNESS TAKES A TRANSACTION-LEVEL LOCK, not a session one, and that is not a detail: through
 * a transaction pooler a session lock would be left on a server connection that is then handed to
 * somebody else, so the probe would poison a key it does not own. `pg_try_advisory_xact_lock`
 * inside an explicit `begin`/`rollback` is one unit of work -- which is exactly what a transaction
 * pooler keeps on one backend -- and PostgreSQL frees it at the end of the transaction whatever
 * happens. Measured: after the witness leg, `pg_locks` showed ONE holder of the probe key, the
 * probe's own, on every configuration above. Transaction-level and session-level advisory locks
 * contend with each other, so the reading is not weakened by using the safer form.
 *
 * THE KEY IS RANDOM PER PROBE. Two application instances booting at once would otherwise collide on
 * a fixed key: the second holder's `pg_try_advisory_lock` would return false and the deployment
 * would be refused for someone else's boot. With a random `objid` the holder's acquisition is the
 * probe's own, so a blocked witness can only mean the holder's lock -- which is what the reading
 * has to mean for the refusal to be about anything.
 *
 * EVERY UNCLEAR ANSWER IS A REFUSAL, in the same direction as the rest of this module: a witness
 * that could not be reached, a holder that could not take its own key, an answer that is not
 * literally false -- none of them SHOW shared lock space, and the whole point of the check is that
 * "not shown" and "shown" are different things.
 *
 * THE NON-OVERRIDE PATH, EXAMINED RATHER THAN ASSUMED. With no override, this function returns
 * `pgConnectionConfig(databaseUrl)` -- the lock connections and the data path are built from ONE
 * string, by one derivation, so there is no second endpoint that could name a different instance
 * and nothing for a comparison to compare. The question Codex raised cannot arise there, and the
 * probe is therefore not run: it would be asking one URL whether it agrees with itself, at the cost
 * of two connections on every deployment. What remains on that path is a DIFFERENT defect, and not
 * one this branch introduced or that the override created: a single hostname that resolves to more
 * than one PostgreSQL (DNS round-robin, or a TCP load balancer over two servers) splits the lock
 * space with one URL. The affinity leg does not see it -- each connection is direct, so each peer
 * comparison passes. Probing it would be a sampling exercise rather than a proof (two connections
 * from one URL catch a two-way split about half the time per boot), which is a different shape of
 * check from this one and belongs with the fan-out sampling the deployment probe already does in
 * `establishStartupOptionByteSafety()`. It is filed as o3d-2vko rather than half built here.
 * -------------------------------------------------------------------------------------------- */

/** How long each of the probe's two connections may spend opening. */
const SHARED_LOCK_SPACE_PROBE_TIMEOUT_MS = 5_000

/**
 * How long any ONE of the probe's statements may take. CLIENT-side (`pg`'s own `query_timeout`),
 * deliberately, not a server `statement_timeout`: a server timeout would have to be carried as a
 * startup option, and this module has already measured what a pooler does to those -- PgBouncer
 * accepts the connection and silently discards them (a `-c statement_timeout=1234` came back
 * `0`). A deadline that a pooler can drop is not a deadline, and the override exists precisely
 * because DATABASE_URL goes through one.
 */
const SHARED_LOCK_SPACE_QUERY_TIMEOUT_MS = 5_000

/**
 * How long the WHOLE probe may take, connects and statements and teardown together (o3d-2k5r r27,
 * Codex MEDIUM).
 *
 * `connectionTimeoutMillis` bounds only the two connects. Everything after them -- the holder's
 * `pg_try_advisory_lock`, the witness's `begin`, the witness query, the `rollback`, the unlock and
 * both `end()`s -- had NO deadline, and this probe is awaited from a lock connection's `onConnect`
 * and now from every lock acquisition. A server or pooler that accepts a socket and then never
 * answers would therefore leave the acquisition pending FOREVER instead of producing the
 * fail-closed refusal this module promises: a money post, an accounting batch, the WMS dispatch
 * sweep or a restore would hang rather than be told what is wrong.
 *
 * 20s is a BACKSTOP, not a competitor to the per-leg timeouts: the worst legitimate path is two
 * 5s connects and one 5s statement, so a probe that reaches this has stalled somewhere the
 * per-leg bounds do not cover (a teardown, a resolver, a stand-in), and the measurement itself is
 * 3-13ms. Expiry is a REFUSAL, in the same direction as every other unclear answer here.
 */
const SHARED_LOCK_SPACE_PROBE_DEADLINE_MS = 20_000

/**
 * The two-int advisory namespace the probe takes its key in.
 *
 * Re-exported by `lib/db/advisory-locks.ts` and listed in `TWO_INT_ADVISORY_LOCK_NAMESPACES` there,
 * so `tests/db/advisory-lock-keys.test.ts` holds it to the same distinctness rule as every other
 * key rather than leaving one namespace declared outside the registry (o3d-4ajo). It is declared
 * HERE because this module is `.mjs` and cannot import the `.ts` registry -- the dependency runs
 * the other way.
 */
export const SESSION_LOCK_SPACE_PROBE_NAMESPACE = 411_220_872

/** The process-wide memo, so the probe is paid once however many lock pools a process builds. */
const SHARED_LOCK_SPACE_SLOT = Symbol.for('ims.db.sharedAdvisoryLockSpace.v1')

/**
 * The memo this PROCESS holds, whichever bundled copy of this module created it.
 *
 * On `globalThis` for the reason the startup-option verdict is (see r20): the built artifact can
 * contain more than one copy of this module, and a module-local `Map` would make each copy pay the
 * probe again. Here that is COST rather than correctness -- a second probe would reach the same
 * verdict -- but paying it twice for no reason is still paying it twice.
 *
 * @returns {Map<string, { startedAt: number, promise: Promise<void> }>}
 */
function sharedLockSpaceMemo() {
  const held = globalThis[SHARED_LOCK_SPACE_SLOT]
  if (held instanceof Map) return held
  const created = new Map()
  Object.defineProperty(globalThis, SHARED_LOCK_SPACE_SLOT, {
    value: created,
    writable: true,
    configurable: true,
    enumerable: false,
...
/** Forget what has been measured. Exported for tests, which must not inherit one another's server. */
export function resetSessionLockSpaceMeasurements() {
  sharedLockSpaceMemo().clear()
}

/**
 * A client config for the probe's own use, with no `onConnect` on it.
 *
 * The hook is dropped deliberately. The probe's two connections are throwaways that ask ONE
 * question -- is this key visible over here -- and an advisory lock is neither schema-scoped nor
 * affected by a startup option, so re-running the data path's own guards on them would only add
 * ways for the probe to fail for reasons that are not its question. The connections the guards are
 * about are checked where they are opened: the lock connection by the affinity leg that has already
 * run before this probe is reached, and the data connections by `pgConnectionConfig()` at their own
 * call sites.
 *
 * @param {string} url
 * @returns {Record<string, unknown>}
 */
function probeConnectionConfig(url) {
  const config = {
    ...pgConnectionConfig(url),
    connectionTimeoutMillis: SHARED_LOCK_SPACE_PROBE_TIMEOUT_MS,
    query_timeout: SHARED_LOCK_SPACE_QUERY_TIMEOUT_MS,
  }
  delete config.onConnect
  return config
}

/**
 * Tear a connection down NOW, rather than asking the server's permission to (o3d-2k5r r27).
 *
 * Used by the probe for its two throwaway clients and by the acquisition deadline below for the
 * ACTUAL lock client, which is the connection the probe exists to license -- see
 * `withLockClientDeadline()`.
 *
 * `end()` is a conversation: it writes a Terminate and waits for the socket to close, which is
 * exactly what a wedged server or pooler will not do -- so the deadline that just fired would be
 * followed by an await that never returns, and the probe would still be holding the connection it
 * gave up on. The SOCKET is destroyed first, unconditionally, which is what actually frees the
 * backend (PostgreSQL rolls back and releases every advisory lock when the connection drops);
 * `end()` is then called for the ordinary case and for stand-ins that have no socket, and both
...
function destroyClientSocket(client) {
  ...
  try {
    const stream = /** @type {{ connection?: { stream?: { destroy?: () => void } } }} */ (client).connection?.stream
    if (typeof stream?.destroy === 'function') stream.destroy()
  } catch { /* a socket that cannot be destroyed is already gone */ }
  try {
    const end = /** @type {{ end?: () => unknown }} */ (client).end
    if (typeof end === 'function') void Promise.resolve(end.call(client)).catch(() => undefined)
  } catch { /* end() on a wedged client is best-effort, which is why the socket went first */ }
}

/**
 * A PostgreSQL boolean, as the two answers that are allowed to mean something.
 *
 * `pg` returns a real boolean; a stand-in may return the wire text. Anything else -- null, a
 * number, an absent column -- is neither, and every caller below treats "neither" as a refusal.
 *
 * @param {unknown} value
...
/**
 * What an operator has to change, appended to every shared-lock-space refusal.
 *
 * @returns {string}
 */
function sharedLockSpaceRemedy() {
  return (
    `WHAT THIS WOULD COST: every session advisory lock in this process -- the money-post lock, the Xero ` +
    `payment write, the WMS dispatch sweep and the restore selection lock -- would be taken somewhere the work ` +
    `never happens, and EVERY holder would be told it holds it. withMoneyPostLock() spans a ledger read and an ` +
    `EXTERNAL money post, so two workers would pay the same document twice with no error anywhere: the ` +
    `duplicate payment is the only symptom. WHAT TO CHANGE: point ${SESSION_LOCK_DATABASE_URL_ENV} at the ` +
    `PostgreSQL that actually serves DATABASE_URL -- the same server, reached on its own host and port instead ` +
    `of the pooler's -- or unset it and let the locks go through DATABASE_URL. DO NOT route around this by ` +
    `removing the check: it is the only thing between "the lock is held" and "the lock is held somewhere that ` +
    `does not matter".`
  )
}

/**
 * MEASURE THE ONE PROPERTY THE OVERRIDE HAS TO HAVE: a lock taken through it is visible to
 * DATABASE_URL.
 *
 * @param {string} databaseUrl the application's DATABASE_URL
 * @param {string} overrideUrl the value of DATABASE_SESSION_LOCK_URL
 * @param {((config: object) => Promise<{ connect(): Promise<unknown>, query(text: string): Promise<{ rows: Array<Record<string, unknown>> }>, end?: () => Promise<unknown> }>) | undefined} createClient
 * @param {(client: unknown) => void} register hands each connection to the deadline the moment it exists
 * @returns {Promise<void>} resolves when the two URLs were SHOWN to share a lock space
 * @throws {DatabaseUrlSchemaConflictError} when they were not, or when it could not be shown
 */
async function runSharedAdvisoryLockSpaceProbe(databaseUrl, overrideUrl, createClient, register) {
  const connect =
    createClient ??
    (async (config) => {
      const pg = await import('pg')
  ...
  // A RANDOM objid, so a second instance booting at the same moment cannot make this refuse. int4:
  // the two-int advisory form takes signed 32-bit arguments.
  const key = 1 + Math.floor(Math.random() * 0x7f_ff_ff_fe)
  const namespace = SESSION_LOCK_SPACE_PROBE_NAMESPACE
   ...
   * @returns {never}
   */
  const unreachable = (leg, error) => {
    throw new DatabaseUrlSchemaConflictError(
      `${SESSION_LOCK_DATABASE_URL_ENV} is set, and whether an advisory lock taken through it is visible to ` +
        `DATABASE_URL could not be established: ${leg} (${probeFailureReason(error)}). Matching database and ` +
        `schema names do not settle it -- a staging server, a restored dump or a pg_basebackup clone carries ` +
        `the same database name, the same schema, the same database OID and even the same pg_control_system() ` +
        `system identifier as production -- so an unanswered question is refused rather than assumed to be a ` +
        `yes. ${sharedLockSpaceRemedy()}`,
    )
  }

  let holder
  try {
    holder = await connect(probeConnectionConfig(overrideUrl))
    // BEFORE `connect()`: a connection the deadline does not know about is a connection it cannot
    // destroy, and `connect()` is one of the things that can stall.
    register(holder)
    await holder.connect()
  } catch (error) {
    await Promise.resolve(holder?.end?.()).catch(() => undefined)
    return unreachable(`the ${SESSION_LOCK_DATABASE_URL_ENV} connection could not be opened`, error)
  }

  try {
    let taken
    try {
      const { rows } = await holder.query(`select pg_try_advisory_lock(${namespace}, ${key}) as taken`)
      taken = postgresBoolean(rows[0]?.taken)
    } catch (error) {
      return unreachable(`the probe lock could not be taken on the ${SESSION_LOCK_DATABASE_URL_ENV} connection`, error)
    }
    if (taken !== true) {
      return unreachable(
        `the ${SESSION_LOCK_DATABASE_URL_ENV} connection did not take its own freshly generated probe key ` +
          `(${namespace}/${key}), answering ${JSON.stringify(String(taken))}, so there is no held lock for the ` +
          `DATABASE_URL side to be blocked by and the reading would mean nothing`,
        new Error('pg_try_advisory_lock did not return true'),
      )
    }
    ...
    let acquired
    try {
      witness = await connect(probeConnectionConfig(databaseUrl))
      register(witness)
      await witness.connect()
      try {
        // EXPLICIT begin/rollback: one unit of work, which is what a transaction pooler keeps on a
        // single backend, and a transaction-level lock PostgreSQL frees at its end whatever happens
        // -- so the probe cannot leave a lock on a server connection somebody else is handed next.
        await witness.query('begin')
        const { rows } = await witness.query(`select pg_try_advisory_xact_lock(${namespace}, ${key}) as acquired`)
        acquired = postgresBoolean(rows[0]?.acquired)
      } finally {
        await Promise.resolve(witness.query('rollback')).catch(() => undefined)
      }
    } catch (error) {
      return unreachable('the DATABASE_URL connection could not be asked for the same key', error)
    } finally {
      await Promise.resolve(witness?.end?.()).catch(() => undefined)
    }

    if (acquired === false) return

    throw new DatabaseUrlSchemaConflictError(
      `${SESSION_LOCK_DATABASE_URL_ENV} does NOT share an advisory-lock space with DATABASE_URL, so the ` +
        `exclusion every session lock in this process depends on would exclude nobody. MEASURED, not inferred: ` +
        `a session advisory lock was taken on a connection made from ${SESSION_LOCK_DATABASE_URL_ENV} ` +
        `(${namespace}/${key}), and a connection made from DATABASE_URL then answered ` +
        `${JSON.stringify(String(acquired))} when asked for the SAME key -- it acquired it, or could not say it ` +
        `was blocked, instead of being blocked by it. PostgreSQL advisory locks live in ONE database on ONE ` +
        `server, so the two URLs are reaching different PostgreSQL instances, or different databases on one. ` +
        `Matching names cannot rule that out and did not: both URLs name the same database and resolve to the ` +
        `same schema, and a staging server, a restored dump or a pg_basebackup clone carries the same database ` +
        `name, the same schema, the same database OID and even the same pg_control_system() system identifier ` +
        `as production -- which is why this is TESTED rather than compared. ${sharedLockSpaceRemedy()}`,
    )
  } finally {
    await Promise.resolve(holder.query(`select pg_advisory_unlock(${namespace}, ${key})`)).catch(() => undefined)
    await Promise.resolve(holder.end?.()).catch(() => undefined)
  }
}

/**
 * THE SAME PROBE, WITH AN END TO IT (o3d-2k5r r27, Codex MEDIUM).
 *
 * Every leg of the probe is bounded on its own -- `connectionTimeoutMillis` on the two connects,
 * `query_timeout` on the statements -- and this is the backstop over the whole of it, including
 * the parts no per-leg option covers: the teardown, and any stand-in or driver that ignores those
 * options. On expiry BOTH clients are destroyed at the socket (see `destroyClientSocket()`, and
 * note that dropping the holder's connection is also what releases the probe key it is holding),
 * and the result is a REFUSAL carrying the same remedy as every other unclear answer, because a
 * question that was never answered has not shown shared lock space.
 *
 * The abandoned probe is left to unwind on its own: its `finally` blocks run against clients whose
 * sockets are already gone, so they resolve or reject into nothing. The verdict has already been
 * decided by then, and waiting for a wedged connection to agree is the failure being fixed.
 *
 ...
 */
async function measureSharedAdvisoryLockSpace(databaseUrl, overrideUrl, createClient, deadlineMs) {
  /** @type {Set<unknown>} */
  ...
  const deadline = new Promise((_resolve, reject) => {
    timer = setTimeout(() => {
      expired = true
      for (const client of opened) destroyClientSocket(client)
      opened.clear()
      reject(
        new DatabaseUrlSchemaConflictError(
          `${SESSION_LOCK_DATABASE_URL_ENV} is set, and whether an advisory lock taken through it is visible to ` +
            `DATABASE_URL could not be established within ${deadlineMs}ms: a connection was opened and then stopped ` +
            `answering, so the probe was given up on and both of its connections were destroyed. A server or ` +
            `pooler that accepts a socket and never replies is indistinguishable from one that is about to say ` +
            `"yes", and this check may only be passed by an answer. ${sharedLockSpaceRemedy()}`,
        ),
      )
    }, deadlineMs)
    // NOT unref()'d, deliberately. An unref'd timer does not hold the event loop open, so a probe
    // that stalls with nothing else pending would let the process exit before the deadline it is
    // relying on ever fires -- the deadline would be there and would never be reached. It is
    // cleared in the `finally` below on every path, so what it holds open is exactly the probe.
  })
  // The loser of the race still settles; without a handler a rejected deadline would surface as an
  // unhandled rejection long after the probe has been admitted.
  deadline.catch(() => undefined)
  try {
    return await /** @type {Promise<void>} */ (
      Promise.race([
        runSharedAdvisoryLockSpaceProbe(databaseUrl, overrideUrl, createClient, (client) => {
          // A connection that arrives AFTER the deadline fired is destroyed on the spot rather
          // than added to a set nothing will read again.
          if (expired) destroyClientSocket(client)
          else opened.add(client)
        }),
        deadline,
      ])
    )
...
/**
 * THE SAME MEASUREMENT, REUSED ONLY WHILE IT IS STILL THE MEASUREMENT THIS CALLER ASKED FOR.
 *
 * `notBefore` is the whole of the policy, and it is what r27 changed. A caller passes the moment
 * its own work began; a memoised verdict is reused only if the probe that produced it STARTED at
 * or after that moment. So:
 *
 *   • a lock ACQUISITION passes `Date.now()` taken before it opens its connection, and therefore
 *     accepts only a probe run for this acquisition -- either the one its own new connection's
 *     `onConnect` just ran, or one it runs itself. It never inherits a verdict from an earlier
 *     acquisition, which is the point (see `sessionLockSpaceReestablisher()`).
 *   • the per-connection `onConnect` leg passes 0 and so reuses whatever is there. It is not the
 *     authority; the acquisition check that follows it immediately is. Passing 0 is what stops a
 *     brand-new physical connection paying for the probe twice in one acquisition.
 *
 * The PROMISE is memoised rather than its result, so callers racing on the same instant share one
 * probe instead of running several. A rejection is memoised on the same terms as a success -- it
 * is reused by an `onConnect` (fail-closed, and no connection storm behind a broken endpoint) and
 * re-measured by the next acquisition, which is the direction that cannot turn a transient
 * unreachable witness into a permanently wedged process.
 *
 * The residue, stated: `Date.now()` has millisecond resolution, so a probe that started in the
 * same millisecond as -- but marginally before -- an acquisition is reused by it. That is a
 * sub-millisecond window against a check whose value is measured in the seconds and days it
 * removes.
 *
 * @param {string} databaseUrl
 * @param {string} overrideUrl
 * @param {{ createClient?: (config: object) => Promise<{ connect(): Promise<unknown>, query(text: string): Promise<{ rows: Array<Record<string, unknown>> }>, end?: () => Promise<unknown> }>, probeDeadlineMs?: number }} options
 * @param {number} notBefore the earliest start time this caller will accept a verdict from
 * @returns {Promise<void>}
 */
function sharedAdvisoryLockSpaceEstablished(databaseUrl, overrideUrl, options, notBefore) {
  const memo = sharedLockSpaceMemo()
  const slot = `${databaseUrl} ${overrideUrl}`
  const held = memo.get(slot)
  if (held !== undefined && held.startedAt >= notBefore) return held.promise
  const startedAt = Date.now()
  const running = measureSharedAdvisoryLockSpace(
    databaseUrl,
    overrideUrl,
    options.createClient,
    options.probeDeadlineMs ?? SHARED_LOCK_SPACE_PROBE_DEADLINE_MS,
  )
  // The memo holds rejections; without a handler of its own a memoised refusal that nobody awaits
  // again would be reported as an unhandled rejection.
  running.catch(() => undefined)
  memo.set(slot, { startedAt, promise: running })
  return running
}

/**
 * RE-ESTABLISH THE SHARED LOCK SPACE FOR ONE LOCK ACQUISITION (o3d-2k5r r27, Codex HIGH).
 *
 * Returns `null` when no override is set -- there is then one URL, one derivation and nothing for
 * a second endpoint to disagree with, so there is no probe and nothing to pay.
 *
 * WHAT THIS DOES AND DOES NOT BUY, because it must not be read as closing the finding it responds
 * to. Codex's objection was that a verdict measured once at boot goes on authorising money
 * movement for the whole life of the process, so a mid-life failover or a re-pointed pooler leaves
 * old and new processes locking different servers with no one told. Re-measuring per acquisition
 * NARROWS that window from the process's lifetime to the milliseconds between this probe and the
 * `pg_try_advisory_lock` it licenses. It does not remove it, and it cannot: any check is a SAMPLE
 * taken before the thing it licenses, and a sample is not a property. What actually removes it is
 * durable, fenced state written through the authoritative `DATABASE_URL` transaction -- filed as
 * o3d-ic9a and deliberately not attempted here.
 *
 * WHAT IT COSTS. Two throwaway connections and four statements per lock acquisition, measured on
 * this host at 3-13ms for the pair. It is paid ONLY by deployments that set the override -- which
 * are exactly the deployments that have two endpoints to disagree -- and only by the four session
 * lock holders, which are low-frequency jobs (a money post, a Xero payment write, the WMS dispatch
 * sweep, a restore), never by the data path and never per query.
 *
 * @param {string | undefined | null} databaseUrl
 * @param {string | undefined | null} sessionLockUrl
 * @param {{ createClient?: (config: object) => Promise<{ connect(): Promise<unknown>, query(text: string): Promise<{ rows: Array<Record<string, unknown>> }>, end?: () => Promise<unknown> }>, probeDeadlineMs?: number }} [options]
 * @returns {((notBefore: number) => Promise<void>) | null}
 */
export function sessionLockSpaceReestablisher(databaseUrl, sessionLockUrl, options = {}) {
  const override = typeof sessionLockUrl === 'string' && sessionLockUrl.trim() !== '' ? sessionLockUrl.trim() : null
  if (override === null) return null
  const base = String(databaseUrl ?? '')
  return (notBefore) =>
    sharedAdvisoryLockSpaceEstablished(base, override, options, typeof notBefore === 'number' ? notBefore : Date.now())
}

/**
 * THE CONNECTION CONFIG FOR A POOL OR CLIENT THAT WILL TAKE A SESSION ADVISORY LOCK.
 *
 * Everything `pgConnectionConfig()` gives the data path — the `search_path` pin, the folded startup
 * options, the non-ASCII backend verdict where one is owed — plus the affinity requirement, which
 * here is unconditional. See the block comment above for why that is a property of this function
 * rather than of the call site.
 *
 * `sessionLockUrl` (`DATABASE_SESSION_LOCK_URL`) is the route past a pooler. It is checked against
 * `databaseUrl` rather than trusted, in TWO steps that answer different questions:
 *
 *   • SYNCHRONOUSLY, here: an override naming a DIFFERENT database or resolving to a DIFFERENT
 *     schema is refused outright. Host and port are deliberately NOT compared — differing is the
 *     whole point of it. These comparisons cost nothing and give a specific message for the two
 *     mistakes a name can express, and they are kept for that; what they CANNOT do is establish
 *     that the two URLs reach the same PostgreSQL, since matching names are not an identity.
 *   • AT RUNTIME, on every lock ACQUISITION (r27): the override must be SHOWN to share an
 *     advisory-lock space with DATABASE_URL. See the block comment above for what is measured, why
 *     a system identifier or a database OID is not the thing to compare, what it costs, and why the
 *     question does not arise when no override is set.
 *
 * The residue of the name comparison is stated rather than hidden: a pooler that ALIASES the
 * database name (PgBouncer's `[databases] ims = dbname=ims_prod`) makes the two URLs name different
 * databases legitimately, and such a deployment must spell the real name in DATABASE_URL too or set
 * no override and connect directly.
 *
 * @param {string | undefined | null} databaseUrl the application's DATABASE_URL
 * @param {string | undefined | null} [sessionLockUrl] the optional direct override
 * @param {string} [purpose] what the lock is for, so a refusal names it
 * @param {{ createClient?: (config: object) => Promise<{ connect(): Promise<unknown>, query(text: string): Promise<{ rows: Array<Record<string, unknown>> }>, end?: () => Promise<unknown> }>, probeDeadlineMs?: number, peerQueryTimeoutMs?: number, acquisitionDeadlineMs?: number }} [options] a stand-in connector and shortened deadlines, for tests
 * @returns {{ connectionString: string, options?: string, onConnect: (client: { query(text: string): Promise<{ rows: Array<Record<string, unknown>> }> }) => Promise<void> }}
 * @throws {DatabaseUrlSchemaConflictError} when the URL names two schemas, or the override disagrees
 */
export function pgSessionLockConnectionConfig(databaseUrl, sessionLockUrl, purpose = 'a session advisory lock', options = {}) {
  const override = typeof sessionLockUrl === 'string' && sessionLockUrl.trim() !== '' ? sessionLockUrl.trim() : null
  if (override !== null) {
    const base = String(databaseUrl ?? '')
    const overrideDatabase = databaseNameOf(override)
    if (overrideDatabase === null) {
      throw new DatabaseUrlSchemaConflictError(
        `${SESSION_LOCK_DATABASE_URL_ENV} is set but is not a database URL naming a database. It must be a ` +
          `direct, non-pooled URL for the same database and schema as DATABASE_URL. ${sessionLockDirectRoute()}`,
      )
    }
    const baseDatabase = databaseNameOf(base)
    if (baseDatabase !== null && overrideDatabase !== baseDatabase) {
      throw new DatabaseUrlSchemaConflictError(
        `${SESSION_LOCK_DATABASE_URL_ENV} names the database ${JSON.stringify(overrideDatabase)} and DATABASE_URL names ` +
          `${JSON.stringify(baseDatabase)}. An advisory lock taken in a different database excludes nothing in the one ` +
          `the work happens in, so the override is refused rather than used. It exists ONLY to reach the SAME ` +
          `database without a pooler in the way.`,
      )
    }
    const overrideSchema = resolveDatabaseUrlSchema(override).schema
    const baseSchema = resolveDatabaseUrlSchema(base).schema
    if (baseSchema !== null && overrideSchema !== baseSchema) {
      throw new DatabaseUrlSchemaConflictError(
        `${SESSION_LOCK_DATABASE_URL_ENV} resolves to the schema ${JSON.stringify(overrideSchema)} and DATABASE_URL ` +
          `resolves to ${JSON.stringify(baseSchema)}. The lock connection must be pinned to the schema the work is ` +
          `written to; spell the same ?schema= on both.`,
      )
    }
  }
  const config = pgConnectionConfig(override ?? databaseUrl)
  // WITHOUT an override there is one URL, so there is nothing for a second endpoint to disagree
  // with and the probe is not attached at all — no connection, no round trip, nothing on the
  // ordinary path. WITH one, the measurement is deferred into `onConnect` because it is async and
  // this function is not, and because that is the moment a lock connection is actually being
  // established: an operator who set it wrongly is told at the first lock, not at import time. The
  // ACQUISITION-level re-measurement is wired separately, by `sessionLockSpaceReestablisher()`,
  // because a connection's `onConnect` fires once per connection and a lock is taken many times on
  // one -- see that function and the block above for what re-measuring per acquisition does and
  // does not buy.
  const sharedLockSpace =
    override === null
      ? null
      : () => sharedAdvisoryLockSpaceEstablished(String(databaseUrl ?? ''), override, options, 0)
  return {
    ...config,
    onConnect: sessionLockAffinityGuard(config.onConnect, purpose, sharedLockSpace, {
      peerQueryTimeoutMs: options.peerQueryTimeoutMs,
