...
/** How long the probe may spend opening each of its two connections. */
 ...
 *
 * @param {string | undefined | null} databaseUrl
 * @param {{ createClient?: (config: object) => { connect(): Promise<unknown>, query(text: string): Promise<{ rows: Array<Record<string, unknown>> }>, end(): Promise<unknown> } }} [options]
...
export async function establishStartupOptionByteSafety(databaseUrl, options = {}) {
  ...
  const settle = (fields) => {
    startupOptionByteVerdict = Object.freeze({ ...NO_VERDICT, probed, target, ...fields })
    return startupOptionByteVerdict
  }
  ...
  let lcCtype = null
  try {
    const [row] = await ask({ connectionString }, [
      'select pg_encoding_to_char(encoding) as server_encoding, datctype as lc_ctype from pg_database where datname = current_database()',
    ])
  ...
  try {
    ...
    return settle({
      ...
      serverEncoding,
      lcCtype,
      reason: `measured on this server (server_encoding=${serverEncoding}, lc_ctype=${lcCtype}): the probed characters survive pg_split_opts() intact`,
    })
  } catch (error) {
...
function nonAsciiOptionByteIsCarried(character, databaseUrl) {
  const verdict = startupOptionByteVerdict
...
function nonAsciiUpgradePath() {
  ...
  const measured =
    verdict.serverEncoding || verdict.lcCtype
      ? ` The deployment probe reported server_encoding=${verdict.serverEncoding} and lc_ctype=${verdict.lcCtype}.`
      : ''
  return (
