 ...
 */
function sessionLockAffinityGuard(startupOptionGuard, purpose) {
  return async (client) => {
 ...
 * `databaseUrl` rather than trusted: an override naming a DIFFERENT database or resolving to a
 * DIFFERENT schema would take the exclusion somewhere other than where the work happens, which is
 * the defect, not the fix. Host and port are deliberately NOT compared — differing is the whole
 * point of it. The residue is stated rather than hidden: a pooler that ALIASES the database name
 * (PgBouncer's `[databases] ims = dbname=ims_prod`) makes the two URLs name different databases
 * legitimately, and such a deployment must spell the real name in DATABASE_URL too or set no
 * override and connect directly.
 *
 ...
 */
export function pgSessionLockConnectionConfig(databaseUrl, sessionLockUrl, purpose = 'a session advisory lock') {
  ...
  return { ...config, onConnect: sessionLockAffinityGuard(config.onConnect, purpose) }
}
