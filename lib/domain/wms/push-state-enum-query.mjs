 ...
 *     they align it deliberately — see `pgConnectionConfig`.)
 ...
 *
 * RE-EXPORTED, NOT DEFINED HERE (o3d-2k5r r8). Prisma reads `?schema=` out of `DATABASE_URL` and
 * sets `search_path` from it; `pg` does not know that parameter, so a raw client on the identical
 * URL can resolve `wms_order_push_links` to a different table — or to none — than the application
 * does. Since the whole point of the query above is that it resolves the table the way the writer
 * does, the two out-of-process gates must not be left on a different search path from the process
 * they are vouching for.
 *
 * The previous round defined this function HERE and applied it to those two gates only, which
 * aligned them with each other and left the RUNTIME — the thing being vouched for — on the server
 * default. The derivation now lives in lib/db/database-url-schema.mjs, next to the adapter and pool
 * that must honour it, and this module re-exports it so the WMS gates keep reaching the rule
 * through the same import they always did. See that file for the full four-consumer story.
 */
export { databaseUrlSchema, pgConnectionConfig, pinClientToMeasuredBackend } from '../../db/database-url-schema.mjs'
