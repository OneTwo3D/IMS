 ...
 *
 * WHAT WENT WRONG (Codex r18, HIGH). `npm run test:db` set `RUN_DB_MIGRATION_TESTS=1` and its
 * tripwire, and `.github/workflows/schema-guardrails.yml` ran that script against a migrated
 * `postgres:16`. But `tests/db/shopping-webhook-retention-evidence.test.ts` gates on a DIFFERENT
 * variable, `RUN_DB_RETENTION_TESTS`, which nothing set. Its one test — the only executable evidence
 * that the WooCommerce order-payload hold actually holds — reported `# SKIP` inside a green job, with
 * no message, for as long as that job existed. The wiring was fixed for one gate and the next gate
 * inherited nothing.
 *
