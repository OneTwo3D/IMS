...
# the end of every ordinary local run (o3d-zzgp r6). Its files seed rows and install DDL, so
# every one of them (o3d-yvn8: via the --import preload in `npm run test:concurrency`)
# refuses, through tests/concurrency/scratch-database-guard.ts, any database that is not
