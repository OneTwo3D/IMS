...
# THE THREE ROOTS AS A LIST, for the one question that has to be asked of all of them at once
# (o3d-secops r7 seventh pass, Codex HIGH): refuse_symlinked_root() will not print a bind-mount
# command whose target overlaps a root this installer also manages, and it reads them from here.
# deploy.sh and update.sh carry the same function and do NOT set this, so the loop there runs zero
# times and the text stays byte-identical in all three.
