...
copy_tree_into_new_dir() {
  ...
  privileged_spare_running_tree "${dest}" "the directory copy_tree_into_new_dir would replace" || die "${IMS_DRIVER_OVERLAP_REASON:-the running-tree check is not loaded, so ${dest} cannot be shown to be separate from it}"
