 ...
 */
const ENV_HEREDOC_DEFAULTS = [
  ...new Set(
    [...shippedFunction(INSTALL_SOURCE, 'write_app_env_file').matchAll(/\$\{([A-Za-z_][A-Za-z0-9_]*)[^}]*\}/g)]
      .map((match) => match[1]),
  ),
]
  .map((name) => `${name}="\${${name}-}"`)
...
function envDatabaseUrl(dir: string, file = '.env'): string {
