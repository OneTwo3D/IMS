...
test('a pass that imported nothing and errored on everything is FAILED (no false-complete)', () => {
  ...
  assert.equal(decideInitialImportOutcome({ imported: 0, skipped: 0, errorCount: 6, unrecordedRefusals: 0 }), 'failed')
...
test('any real progress makes it COMPLETE even with some per-order errors', () => {
  assert.equal(decideInitialImportOutcome({ imported: 4, skipped: 0, errorCount: 2, unrecordedRefusals: 0 }), 'complete') // partial import
  assert.equal(decideInitialImportOutcome({ imported: 0, skipped: 3, errorCount: 2, unrecordedRefusals: 0 }), 'complete') // all already imported
  assert.equal(decideInitialImportOutcome({ imported: 5, skipped: 0, errorCount: 0, unrecordedRefusals: 0 }), 'complete') // clean import
})
