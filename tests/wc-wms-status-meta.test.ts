...
test('buildWmsStatusMetaPatch keeps the FIRST id on duplicate keys (matches WC get_meta)', () => {
  const existing = [
    { id: 11, key: '_oti_wms_status', value: 'PICKING' },
    { id: 77, key: '_oti_wms_status', value: 'STALE-DUP' }, // duplicate row from a prior bug
  ]
  const patch = buildWmsStatusMetaPatch(INPUT, existing)
  assert.equal(patch.find((p) => p.key === '_oti_wms_status')?.id, 11) // first, not the trailing dup
})
