...
//
// o3d-e2mz: the where also carries the claimed attemptRevision, so the write is
// pinned to the attempt that failed.
...
test('optimistic update keys on the observed retryCount and reports the applied transition', async () => {
  ...
  const result = await markSyncLogForFollowUpRetry({ id: 'log-1', attemptRevision: 7 }, { retryCount: 2 }, new Error('boom'), client)

  // Compound where prevents a stale double-write.
  assert.deepEqual(calls.updateMany[0].where, { id: 'log-1', attemptRevision: 7, retryCount: 2 })
...
test('reaching MAX_RETRIES marks the row FAILED', async () => {
  ...
  const result = await markSyncLogForFollowUpRetry({ id: 'log-2', attemptRevision: 1 }, { retryCount: 4 }, new Error('boom'), client)
...
test('lost race (count 0) reflects the PERSISTED terminal state, not the stale view', async () => {
  ...
  const result = await markSyncLogForFollowUpRetry({ id: 'log-3', attemptRevision: 1 }, { retryCount: 1 }, new Error('boom'), client)
...
test('lost race where the winner left the row retryable reports finalFailure=false', async () => {
  ...
  const result = await markSyncLogForFollowUpRetry({ id: 'log-4', attemptRevision: 1 }, { retryCount: 1 }, new Error('boom'), client)
  assert.equal(result.finalFailure, false)
})
...
test('lost race with a vanished row falls back to the computed view', async () => {
  ...
  const result = await markSyncLogForFollowUpRetry({ id: 'log-5', attemptRevision: 1 }, { retryCount: 4 }, new Error('boom'), client)
