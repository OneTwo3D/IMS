...
/**
 * o3d-i0o6 r5 (Codex round 4, HIGH 3) — THE EVIDENCE THE REAL A1 WRITER LEAVES IS THE EVIDENCE THE
 * LEDGER PROOF READS.
 *
 * A refund's unearned-revenue reversal is pinned to the ledger A1 raised the liability in, and that
 * ledger is resolved through `SalesOrder.revenueDeferredBatchRef` → the batch log's own `connector`.
 * Two links have to hold for that to be worth anything, and neither is checked by a fixture that
 * writes both ends itself:
 *
 *   1. the reference A1 STAMPS is the exact `referenceId` of the log it CREATED (a run crossing
 *      midnight is exactly how those came apart before — o3d-0qoo);
 *   2. that log carries the connector of the writer that raised it.
 *
 * So this drives the REAL A1 group of each connector and hands what it left behind to the real
 * proof. A fixture in refund-service.test.ts describes this shape; this is what says the shape is
 * one the writer actually produces.
 */
for (const connector of CONNECTORS) {
  test(`${connector.name} Group A1's stamped batch reference resolves to ITS OWN ledger (o3d-i0o6 r5)`, async () => {
    const { runDailyBatchSync } = await connector.load()
    reset({ snapshotDiscount: 10 })

    const result = await runDailyBatchSync()
    assert.deepEqual(result.errors, [])
    assert.equal(result.groupA1, 1, 'the A1 group really did stage the order')

    const journal = world.created.find((log) => log.type === 'DAILY_BATCH_REVENUE_DEFERRAL')
    assert.ok(journal, 'A1 raised a deferral journal')
    const stamped = world.orderUpdates.find((update) => 'revenueDeferredBatchRef' in update.data)
    assert.ok(stamped, 'and stamped the order with a batch reference')
    assert.equal(
      stamped.data.revenueDeferredBatchRef,
      journal.referenceId,
      'the stamped reference IS the created log\'s id — the link the proof walks',
    )
    const deferred = Number(stamped.data.unearnedRevenueAmount)
    assert.ok(deferred > 0, `and a real deferral amount: ${deferred}`)

    const { proveRevenueDeferralLedger } = await import('@/lib/domain/accounting/revenue-deferral-ledger-proof')
    // The one row that reference resolves to, with the connector the writer stamped on it.
    const client = {
      accountingSyncLog: {
        findMany: async ({ where }: { where: { type: string; referenceType: string; referenceId: string } }) => (
          where.type === 'DAILY_BATCH_REVENUE_DEFERRAL'
            && where.referenceType === 'DailyBatch'
            && where.referenceId === journal.referenceId
            ? [{ connector: connector.name, status: 'PENDING' }]
            : []
        ),
      },
    }
    const proof = await proveRevenueDeferralLedger(
      client,
      {
        revenueDeferredDate: stamped.data.revenueDeferredDate as Date,
        revenueDeferredBatchRef: stamped.data.revenueDeferredBatchRef as string,
        unearnedRevenueAmount: deferred,
      },
      ['xero', 'quickbooks'] as const,
    )
    assert.equal(proof.kind, 'proved', `the ledger is established: ${JSON.stringify(proof)}`)
    assert.equal(proof.kind === 'proved' ? proof.connector : null, connector.name)

    // AND IT IS NOT VACUOUS: a reference that does not resolve is refused, so the `proved` above is
    // the lookup succeeding rather than the function answering the same way to everything.
    const missing = await proveRevenueDeferralLedger(
      client,
      {
        revenueDeferredDate: stamped.data.revenueDeferredDate as Date,
        revenueDeferredBatchRef: 'A1-2026-01-01-00000000',
        unearnedRevenueAmount: deferred,
      },
      ['xero', 'quickbooks'] as const,
    )
    assert.equal(missing.kind, 'unestablished')
  })
}
