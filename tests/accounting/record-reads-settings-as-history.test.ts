...
// MUTATION THAT KILLS THIS (run): restore "open that order in WooCommerce and remove any duplicate
// note" to NON_DOCUMENT_INCIDENT_WORDING.WC_INVOICE_NOTE.remedy, or "Review the tax rates in
// {ledger}, and correct or archive that rate there if this write was wrong" to TAX_RATE_SYNC — each
// equality fails on the whole message, and the fence in record-names-only-what-it-holds.test.ts
// fails alongside it.
//
