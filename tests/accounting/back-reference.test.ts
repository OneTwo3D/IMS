...
type FakeBill = {
  ...
  /** o3d-9kek r3 finding 1: "<connector>:<tenantId>" that issued the id. '' = issuer unknown. */
...
type FakeSyncRow = {
  ...
  /** The connection that issued externalTransactionId. NULL = unknown, and counted fail-closed. */
  provenance?: string | null
}

/** The tenant/realm namespaces these tests reason in. Two QuickBooks realms, ids that collide. */
const XERO_A = 'xero:tenant-A'
const QBO_A = 'quickbooks:realm-A'
const QBO_B = 'quickbooks:realm-B'

const SYNC_ROW_COLUMNS = new Set(['connector', 'type', 'referenceType', 'referenceId', 'status', 'externalTransactionId', 'provenance'])
const BILL_COLUMNS = new Set(['poId', 'accountingInvoiceId', 'accountingInvoiceProvenance', 'id'])
...
function makeDeps(overrides: {
  ...
  /** Namespace of purchaseInvoiceAccountingInvoiceId. '' = issued by an unidentifiable connection. */
  purchaseInvoiceProvenance?: string
  ...
  const enforceExternalIdUniqueness = (data: Record<string, unknown>, writtenBillIds: string[]) => {
    ...
    // Over the PAIR, exactly as the database index is (o3d-9kek r3 finding 1). A double that kept
    // enforcing the old value-only index would make the realm-collision test fail for the right
    // reason by accident, and — worse — would hide a production that never stored provenance at all.
  ...
  const deps: BackReferenceDeps = {
    ...
    purchaseInvoice: {
      ...
      async findUnique() {
        return {
          accountingInvoiceId: overrides.purchaseInvoiceAccountingInvoiceId ?? null,
          accountingInvoiceProvenance: overrides.purchaseInvoiceProvenance ?? '',
        }
      },
...
function selfRow(poId: string, externalTransactionId: string | null, status = 'SYNCED', provenance: string | null = XERO_A): FakeSyncRow {
  return { id: 'log-self', connector: 'xero', type: 'PURCHASE_INVOICE', referenceType: 'PurchaseOrder', referenceId: poId, status, externalTransactionId, provenance }
}
...
test('applyBackReference writes the external id onto a sales order', async () => {
  ...
  await applyBackReference(deps, { connector: 'xero', provenance: XERO_A, type: 'SALES_INVOICE', referenceType: 'SalesOrder', referenceId: 'so-1', externalId: 'XINV-1', invoiceNumber: 'INV-100' })
...
test('applyBackReference PROPAGATES (does not swallow) a write failure so the caller can retry', async () => {
  ...
  await assert.rejects(
    () => applyBackReference(deps, { connector: 'xero', provenance: XERO_A, type: 'SALES_INVOICE', referenceType: 'SalesOrder', referenceId: 'so-1', externalId: 'XINV-1' }),
...
test('backReferenceIsMissing is true when the document lacks the external id, false when set', async () => {
  ...
  assert.equal(await backReferenceIsMissing(missing.deps, { connector: 'xero', provenance: XERO_A, type: 'SALES_INVOICE', referenceType: 'SalesOrder', referenceId: 'so-1', externalId: 'XINV-1' }), true)

  const present = makeDeps({ salesOrderAccountingInvoiceId: 'XINV-1' })
  assert.equal(await backReferenceIsMissing(present.deps, { connector: 'xero', provenance: XERO_A, type: 'SALES_INVOICE', referenceType: 'SalesOrder', referenceId: 'so-1', externalId: 'XINV-1' }), false)
})
...
test('repair flow: a document orphaned by a back-reference failure is detected and re-applied', async () => {
  ...
  await assert.rejects(() => applyBackReference(failing.deps, { connector: 'xero', provenance: XERO_A, type: 'SALES_INVOICE', referenceType: 'SalesOrder', referenceId: 'so-1', externalId: 'XINV-1' }))

  // 2) repair sweep later: the document still lacks the id...
  const repair = makeDeps({ salesOrderAccountingInvoiceId: null })
  assert.equal(await backReferenceIsMissing(repair.deps, { connector: 'xero', provenance: XERO_A, type: 'SALES_INVOICE', referenceType: 'SalesOrder', referenceId: 'so-1', externalId: 'XINV-1' }), true)
  // ...so it re-applies from the stored external id and succeeds.
  await applyBackReference(repair.deps, { connector: 'xero', provenance: XERO_A, type: 'SALES_INVOICE', referenceType: 'SalesOrder', referenceId: 'so-1', externalId: 'XINV-1' })
...
test('backReferenceIsMissing for PURCHASE_INVOICE/PurchaseOrder reflects an unlinked bill', async () => {
  ...
  const hasNull = makeDeps({ bills: [{ id: 'pi-1', poId: 'po-1', accountingInvoiceId: null, accountingInvoiceProvenance: '', createdAt: 1 }] })
  assert.equal(await backReferenceIsMissing(hasNull.deps, { connector: 'xero', provenance: XERO_A, type: 'PURCHASE_INVOICE', referenceType: 'PurchaseOrder', referenceId: 'po-1', externalId: 'XBILL-1' }), true)

  const allLinked = makeDeps({ bills: [{ id: 'pi-1', poId: 'po-1', accountingInvoiceId: 'XBILL-other', accountingInvoiceProvenance: XERO_A, createdAt: 1 }] })
  assert.equal(await backReferenceIsMissing(allLinked.deps, { connector: 'xero', provenance: XERO_A, type: 'PURCHASE_INVOICE', referenceType: 'PurchaseOrder', referenceId: 'po-1', externalId: 'XBILL-1' }), false)
})
...
test('PURCHASE_CREDIT_NOTE/SupplierCreditNote writes accountingCreditNoteId back (g5u2)', async () => {
  ...
  const d = makeDeps({})
  await applyBackReference(d.deps, { connector: 'xero', provenance: XERO_A, type: 'PURCHASE_CREDIT_NOTE', referenceType: 'SupplierCreditNote', referenceId: 'scn-1', externalId: 'XCN-9' })
  ...
  assert.equal(await backReferenceIsMissing(missing.deps, { connector: 'xero', provenance: XERO_A, type: 'PURCHASE_CREDIT_NOTE', referenceType: 'SupplierCreditNote', referenceId: 'scn-1', externalId: 'XCN-9' }), true)
  const linked = makeDeps({ supplierCreditNoteAccountingCreditNoteId: 'XCN-9' })
  assert.equal(await backReferenceIsMissing(linked.deps, { connector: 'xero', provenance: XERO_A, type: 'PURCHASE_CREDIT_NOTE', referenceType: 'SupplierCreditNote', referenceId: 'scn-1', externalId: 'XCN-9' }), false)
})
...
test('resolvePurchaseOrderBackReference counts the whole population for the PO, not a page', async () => {
  const { deps, calls } = makeDeps({
    bills: [{ id: 'bill-1', poId: 'po-1', accountingInvoiceId: null, accountingInvoiceProvenance: '', createdAt: 1 }],
    poSyncRows: [selfRow('po-1', 'XBILL-1')],
  })
  const resolved = await resolvePurchaseOrderBackReference(deps, { connector: 'xero', provenance: XERO_A, purchaseOrderId: 'po-1', externalId: 'XBILL-1' })
...
test('resolvePurchaseOrderBackReference is ambiguous when another sync row references the PO', async () => {
  const { deps } = makeDeps({
    bills: [{ id: 'bill-1', poId: 'po-1', accountingInvoiceId: null, accountingInvoiceProvenance: '', createdAt: 1 }],
    poSyncRows: [selfRow('po-1', 'XBILL-1'), { id: 'log-other', connector: 'xero', type: 'PURCHASE_INVOICE', referenceType: 'PurchaseOrder', referenceId: 'po-1', status: 'SYNCED', externalTransactionId: 'XBILL-2', provenance: XERO_A }],
  })
  const resolved = await resolvePurchaseOrderBackReference(deps, { connector: 'xero', provenance: XERO_A, purchaseOrderId: 'po-1', externalId: 'XBILL-1' })
...
test('resolvePurchaseOrderBackReference is ambiguous when the PO has several unlinked bills', async () => {
  const { deps } = makeDeps({
    bills: [
      { id: 'bill-1', poId: 'po-1', accountingInvoiceId: null, accountingInvoiceProvenance: '', createdAt: 1 },
      { id: 'bill-2', poId: 'po-1', accountingInvoiceId: null, accountingInvoiceProvenance: '', createdAt: 2 },
    ],
    poSyncRows: [selfRow('po-1', 'XBILL-1')],
  })
  const resolved = await resolvePurchaseOrderBackReference(deps, { connector: 'xero', provenance: XERO_A, purchaseOrderId: 'po-1', externalId: 'XBILL-1' })
...
test('[o3d-9kek f1] an external id already on a bill of the PO is already-linked, never re-attributed', async () => {
  ...
  const { deps, calls } = makeDeps({
    bills: [
      { id: 'bill-a', poId: 'po-1', accountingInvoiceId: 'XBILL-1', accountingInvoiceProvenance: XERO_A, createdAt: 1 },
      { id: 'bill-b', poId: 'po-1', accountingInvoiceId: null, accountingInvoiceProvenance: '', createdAt: 9 },
    ],
    poSyncRows: [selfRow('po-1', 'XBILL-1')],
  })
  const resolved = await resolvePurchaseOrderBackReference(deps, { connector: 'xero', provenance: XERO_A, purchaseOrderId: 'po-1', externalId: 'XBILL-1' })
  ...
  const applied = await applyBackReference(deps, { connector: 'xero', provenance: XERO_A, type: 'PURCHASE_INVOICE', referenceType: 'PurchaseOrder', referenceId: 'po-1', externalId: 'XBILL-1' })
...
test('applyBackReference REFUSES to guess which bill an ambiguous PO row belongs to', async () => {
  const { deps, calls } = makeDeps({
    bills: [
      { id: 'bill-old', poId: 'po-1', accountingInvoiceId: null, accountingInvoiceProvenance: '', createdAt: 1 },
      { id: 'bill-new', poId: 'po-1', accountingInvoiceId: null, accountingInvoiceProvenance: '', createdAt: 9 },
    ],
    poSyncRows: [selfRow('po-1', 'XBILL-1')],
  })
  const applied = await applyBackReference(deps, { connector: 'xero', provenance: XERO_A, type: 'PURCHASE_INVOICE', referenceType: 'PurchaseOrder', referenceId: 'po-1', externalId: 'XBILL-1' })
...
test('applyBackReference writes an unambiguous PO row onto that exact bill', async () => {
  const { deps, calls } = makeDeps({
    bills: [
      { id: 'bill-1', poId: 'po-1', accountingInvoiceId: null, accountingInvoiceProvenance: '', createdAt: 1 },
      { id: 'bill-linked', poId: 'po-1', accountingInvoiceId: 'XBILL-0', accountingInvoiceProvenance: XERO_A, createdAt: 9 },
    ],
    poSyncRows: [selfRow('po-1', 'XBILL-1')],
  })
  const applied = await applyBackReference(deps, { connector: 'xero', provenance: XERO_A, type: 'PURCHASE_INVOICE', referenceType: 'PurchaseOrder', referenceId: 'po-1', externalId: 'XBILL-1' })
...
test('[o3d-9kek f3] a bill linked between the resolve and the write is NOT overwritten', async () => {
  ...
  const applied = await applyBackReference(deps, { connector: 'xero', provenance: XERO_A, type: 'PURCHASE_INVOICE', referenceType: 'PurchaseOrder', referenceId: 'po-1', externalId: 'XBILL-legacy' })
...
test('applyBackReference reports nothing-to-apply when every bill on the PO is already linked', async () => {
  const { deps, calls } = makeDeps({
    bills: [{ id: 'bill-1', poId: 'po-1', accountingInvoiceId: 'XBILL-other', accountingInvoiceProvenance: XERO_A, createdAt: 1 }],
    poSyncRows: [selfRow('po-1', 'XBILL-1')],
  })
  const applied = await applyBackReference(deps, { connector: 'xero', provenance: XERO_A, type: 'PURCHASE_INVOICE', referenceType: 'PurchaseOrder', referenceId: 'po-1', externalId: 'XBILL-1' })
...
test('[o3d-9kek r2 f1] the claim lookup asks the whole table, not just this PO', async () => {
  ...
  await resolvePurchaseOrderBackReference(deps, { connector: 'xero', provenance: XERO_A, purchaseOrderId: 'po-1', externalId: 'XBILL-1' })
  // A poId in this predicate is the defect: it is what made an id held by another order's bill
  // invisible, and the write a silent duplicate. It is scoped by PROVENANCE and nothing else
  // (o3d-9kek r3 finding 1) — the whole table, within one connection's namespace.
  assert.deepEqual(calls.billFindFirstWheres[0], { accountingInvoiceId: 'XBILL-1', accountingInvoiceProvenance: XERO_A })
})
...
test('[o3d-9kek r2 f1] an external id already on ANOTHER PO\'s bill is a conflict, not a free slot', async () => {
  const { deps, calls } = makeDeps({
    bills: [
      ...
      { id: 'bill-elsewhere', poId: 'po-2', accountingInvoiceId: 'XBILL-1', accountingInvoiceProvenance: XERO_A, createdAt: 1 },
      { id: 'bill-here', poId: 'po-1', accountingInvoiceId: null, accountingInvoiceProvenance: '', createdAt: 9 },
    ],
    poSyncRows: [selfRow('po-1', 'XBILL-1')],
  })
...
test('[o3d-9kek r2 f1] a SIBLING bill taking the id after the resolve cannot receive a second copy', async () => {
  ...
  const bills: FakeBill[] = [
    { id: 'bill-a', poId: 'po-1', accountingInvoiceId: null, accountingInvoiceProvenance: '', createdAt: 1 },
    { id: 'bill-b', poId: 'po-1', accountingInvoiceId: 'XBILL-old', accountingInvoiceProvenance: XERO_A, createdAt: 9 },
  ]
  const { deps, calls } = makeDeps({
    bills,
    poSyncRows: [selfRow('po-1', 'XBILL-1')],
...
test('[o3d-9kek r2 f1] the authoritative bill-keyed write refuses a duplicate id with an explanation', async () => {
  ...
  const { deps } = makeDeps({
    bills: [
      { id: 'bill-1', poId: 'po-1', accountingInvoiceId: 'XBILL-1', accountingInvoiceProvenance: XERO_A, createdAt: 1 },
      { id: 'bill-2', poId: 'po-1', accountingInvoiceId: null, accountingInvoiceProvenance: '', createdAt: 2 },
    ],
  })
  await assert.rejects(
    () => applyBackReference(deps, { connector: 'xero', provenance: XERO_A, type: 'PURCHASE_INVOICE', referenceType: 'PurchaseInvoice', referenceId: 'bill-2', externalId: 'XBILL-1' }),
...
test('[o3d-9kek f1] backReferenceIsMissing stops reporting a PO row whose id is already on a bill', async () => {
  ...
  assert.equal(
    await backReferenceIsMissing(alreadyApplied.deps, { connector: 'xero', provenance: XERO_A, type: 'PURCHASE_INVOICE', referenceType: 'PurchaseOrder', referenceId: 'po-1', externalId: 'XBILL-1' }),
    false,
  )

  const genuinelyMissing = makeDeps({
    bills: [{ id: 'bill-b', poId: 'po-1', accountingInvoiceId: null, accountingInvoiceProvenance: '', createdAt: 9 }],
    poSyncRows: [selfRow('po-1', 'XBILL-1')],
  })
  assert.equal(
    await backReferenceIsMissing(genuinelyMissing.deps, { connector: 'xero', provenance: XERO_A, type: 'PURCHASE_INVOICE', referenceType: 'PurchaseOrder', referenceId: 'po-1', externalId: 'XBILL-1' }),
    true,
  )
})

// ---------------------------------------------------------------------------
// o3d-9kek ROUND 3 finding 1 — the unique index was value-only, on the reasoning that a Xero
// GUID cannot collide with a QuickBooks integer. The collision that actually happens is
// QuickBooks realm A against QuickBooks realm B, numeric on BOTH sides. Disconnect keeps
// historical bill ids (they are financial evidence) while dropping the realm pin, so after a
// reconnect to a different company a new bill posts successfully to QuickBooks and then cannot be
// linked locally, because a retired realm's bill holds the same integer.
//
// External ids are therefore stored WITH the connection that issued them, and every question this
// module asks — who holds this id, is this document linked, who competes for this bill — is asked
// inside one namespace.
// ---------------------------------------------------------------------------

/** A live PURCHASE_INVOICE row for a PO, in a named QuickBooks realm. */
function qboRow(id: string, poId: string, externalTransactionId: string | null, provenance: string | null): FakeSyncRow {
  return { id, connector: 'quickbooks', type: 'PURCHASE_INVOICE', referenceType: 'PurchaseOrder', referenceId: poId, status: 'SYNCED', externalTransactionId, provenance }
}

test('[o3d-9kek r3 f1] a retired realm\'s bill holding the SAME integer id is neither a holder nor a conflict', async () => {
  // Bill 1042 in realm A is history: the company it belonged to is disconnected. QuickBooks then
  // issues 1042 again in realm B for an entirely different bill. Under a global unique index the
  // local link is refused forever — the remote document exists and IMS can never point at it.
  const { deps, calls } = makeDeps({
    bills: [
      { id: 'bill-realm-a', poId: 'po-9', accountingInvoiceId: '1042', accountingInvoiceProvenance: QBO_A, createdAt: 1 },
      { id: 'bill-realm-b', poId: 'po-1', accountingInvoiceId: null, accountingInvoiceProvenance: '', createdAt: 9 },
    ],
    poSyncRows: [qboRow('log-b', 'po-1', '1042', QBO_B)],
  })

  const resolved = await resolvePurchaseOrderBackReference(deps, { connector: 'quickbooks', provenance: QBO_B, purchaseOrderId: 'po-1', externalId: '1042' })
  assert.equal(resolved.outcome, 'unique')
  assert.equal(resolved.outcome === 'unique' && resolved.purchaseInvoiceId, 'bill-realm-b')

  const applied = await applyBackReference(deps, { connector: 'quickbooks', provenance: QBO_B, type: 'PURCHASE_INVOICE', referenceType: 'PurchaseOrder', referenceId: 'po-1', externalId: '1042' })
  assert.equal(applied.outcome, 'applied')
  // Both bills carry 1042 and that is CORRECT — they are ids in two different companies. The
  // double enforces the index over the pair, so this passing is evidence the pair is what is
  // stored, not merely that nothing checked.
  assert.deepEqual(calls.lastUpdateData, { accountingInvoiceId: '1042', accountingInvoiceProvenance: QBO_B })
})

test('[o3d-9kek r3 f1] WITHIN one realm the id is still exclusive, and the holder is still reported', async () => {
  // The other half of the same rule: pairing the id with a namespace must not weaken the
  // invariant inside a namespace, which is the one that stops two local bills pointing at one
  // ledger document.
  const { deps, calls } = makeDeps({
    bills: [
      { id: 'bill-holder', poId: 'po-9', accountingInvoiceId: '1042', accountingInvoiceProvenance: QBO_B, createdAt: 1 },
      { id: 'bill-here', poId: 'po-1', accountingInvoiceId: null, accountingInvoiceProvenance: '', createdAt: 9 },
    ],
    poSyncRows: [qboRow('log-b', 'po-1', '1042', QBO_B)],
  })
  ...
  const resolved = await resolvePurchaseOrderBackReference(deps, { connector: 'quickbooks', provenance: QBO_B, purchaseOrderId: 'po-1', externalId: '1042' })
  assert.equal(resolved.outcome === 'ambiguous' && resolved.reason, 'EXTERNAL_ID_LINKED_ELSEWHERE')
  assert.equal(resolved.outcome === 'ambiguous' && resolved.linkedPurchaseInvoiceId, 'bill-holder')
...
test('[o3d-9kek r3 f1] the AUTHORITATIVE bill-keyed write stamps the namespace with the id, in one statement', async () => {
  const { deps, calls } = makeDeps({
    bills: [{ id: 'bill-1', poId: 'po-1', accountingInvoiceId: null, accountingInvoiceProvenance: '', createdAt: 1 }],
  })
  const applied = await applyBackReference(deps, { connector: 'quickbooks', provenance: QBO_B, type: 'PURCHASE_INVOICE', referenceType: 'PurchaseInvoice', referenceId: 'bill-1', externalId: '1042' })
  assert.equal(applied.outcome, 'applied')
  // Together, not in two statements: a window in which the id is stored with the '' sentinel is a
  // window in which the unique index is enforcing the wrong namespace.
  assert.deepEqual(calls.lastUpdateData, { accountingInvoiceId: '1042', accountingInvoiceProvenance: QBO_B })
})

test('[o3d-9kek r3 f1] with no connected tenant nothing is written at all', async () => {
  const { deps, calls } = makeDeps({
    bills: [{ id: 'bill-1', poId: 'po-1', accountingInvoiceId: null, accountingInvoiceProvenance: '', createdAt: 1 }],
    poSyncRows: [qboRow('log-b', 'po-1', '1042', QBO_B)],
  })
  const applied = await applyBackReference(deps, { connector: 'quickbooks', provenance: '', type: 'PURCHASE_INVOICE', referenceType: 'PurchaseOrder', referenceId: 'po-1', externalId: '1042' })
  assert.equal(applied.outcome, 'nothing-to-apply')
  assert.equal(calls.purchaseInvoiceUpdateMany, 0)
  assert.equal(calls.purchaseInvoiceUpdate, 0)
  // Stamping it into the '' namespace would put it where a DIFFERENT company's id can collide
  // with it, and where no later reader could tell which company it came from.
  assert.equal(calls.lastUpdateData, undefined)
})

test('[o3d-9kek r3 f1] a competitor whose issuing connection is UNKNOWN still makes the PO ambiguous', async () => {
  // Fail-closed. A NULL-provenance row posted something; if that something was this realm's bill,
  // ignoring it turns a genuine two-claimant ambiguity into a confident wrong answer.
  const { deps } = makeDeps({
    bills: [{ id: 'bill-1', poId: 'po-1', accountingInvoiceId: null, accountingInvoiceProvenance: '', createdAt: 1 }],
    poSyncRows: [qboRow('log-b', 'po-1', '1042', QBO_B), qboRow('log-legacy', 'po-1', '1043', null)],
  })
  const resolved = await resolvePurchaseOrderBackReference(deps, { connector: 'quickbooks', provenance: QBO_B, purchaseOrderId: 'po-1', externalId: '1042' })
  assert.equal(resolved.outcome === 'ambiguous' && resolved.reason, 'MULTIPLE_SYNC_ROWS')
  assert.equal(resolved.outcome === 'ambiguous' && resolved.syncRowCount, 2)
})

test('[o3d-9kek r3 f1] a competitor from a DIFFERENT realm competes for nothing here', async () => {
  const { deps } = makeDeps({
    bills: [{ id: 'bill-1', poId: 'po-1', accountingInvoiceId: null, accountingInvoiceProvenance: '', createdAt: 1 }],
    poSyncRows: [qboRow('log-b', 'po-1', '1042', QBO_B), qboRow('log-a', 'po-1', '1042', QBO_A)],
  })
  const resolved = await resolvePurchaseOrderBackReference(deps, { connector: 'quickbooks', provenance: QBO_B, purchaseOrderId: 'po-1', externalId: '1042' })
  assert.equal(resolved.outcome, 'unique')
  assert.equal(resolved.outcome === 'unique' && resolved.purchaseInvoiceId, 'bill-1')
})

test('[o3d-9kek r3 f1] a bill linked in a RETIRED realm reads as unlinked in the current one', async () => {
  const stale = makeDeps({ purchaseInvoiceAccountingInvoiceId: '1042', purchaseInvoiceProvenance: QBO_A })
  assert.equal(
    await backReferenceIsMissing(stale.deps, { connector: 'quickbooks', provenance: QBO_B, type: 'PURCHASE_INVOICE', referenceType: 'PurchaseInvoice', referenceId: 'bill-1', externalId: '1042' }),
    true,
    'a link to a company we are no longer talking to is not a link',
  )

  const current = makeDeps({ purchaseInvoiceAccountingInvoiceId: '1042', purchaseInvoiceProvenance: QBO_B })
  assert.equal(
    await backReferenceIsMissing(current.deps, { connector: 'quickbooks', provenance: QBO_B, type: 'PURCHASE_INVOICE', referenceType: 'PurchaseInvoice', referenceId: 'bill-1', externalId: '1042' }),
    false,
  )
})
