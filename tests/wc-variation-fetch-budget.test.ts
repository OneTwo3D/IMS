...
// o3d-jcx: the numbers, checked against each other. Constants that only agree by accident are the
// reason the fetch could outlive the claim on the item it was processing.
...
test('o3d-jcx: the page ceiling and the item ceiling agree', () => {
  // A page ceiling looser than the item ceiling would let the walk spend requests discovering a
  // product it was always going to refuse; a tighter one would refuse products under the item limit.
  assert.equal(MAX_WC_VARIATION_PAGES, Math.ceil(MAX_WC_VARIATIONS_PER_PRODUCT / WC_VARIATION_PAGE_SIZE))
