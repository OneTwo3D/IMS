...
test('a supplier credit note the ledger does not hold is CREATED, with POST (o3d-tfri r3)', async () => {
  ...
  const result = await pushPurchaseCreditNote(base, 'AUTHORISED', { firstAttempt: true, lookup: NOT_IN_LEDGER })
...
test('a replay the ledger cannot vouch for REFUSES rather than creating a second credit note', async () => {
  reset()
  const { pushPurchaseCreditNote } = await creditNotes()
  const result = await pushPurchaseCreditNote(base, 'AUTHORISED', { firstAttempt: false, lookup: NOT_IN_LEDGER })
...
test('the poster runs the LIVE lookup when no seam is supplied, and posts only after it answers', async () => {
  reset({ ok: true, status: 200, data: { CreditNotes: [] } })
  const { pushPurchaseCreditNote } = await creditNotes()
  const result = await pushPurchaseCreditNote(base, 'AUTHORISED', { firstAttempt: true })
...
test('two supplier credits on ONE purchase order still post as two distinct documents', async () => {
  reset()
  const { pushPurchaseCreditNote } = await creditNotes()
  await pushPurchaseCreditNote({ ...base, creditNoteNumber: 'SCN-scn-2' }, 'AUTHORISED', { firstAttempt: true, lookup: NOT_IN_LEDGER })
  await pushPurchaseCreditNote({ ...base, creditNoteNumber: 'SCN-scn-9' }, 'AUTHORISED', { firstAttempt: true, lookup: NOT_IN_LEDGER })
