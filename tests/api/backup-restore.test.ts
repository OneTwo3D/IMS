...
test('production restore POST is disabled by default before parsing restore input', async () => {
  const { deps, calls, activityLogs } = baseDeps()
...
test('uploaded production restore requires the upload kill switch and does not consume the email code', async () => {
