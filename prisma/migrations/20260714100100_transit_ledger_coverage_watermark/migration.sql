-- 6oyu.4 (epic khdw): durable coverage watermark for the transit subledger
-- reconciliation. The transit_subledger_movements ledger is append-only and has NO
-- pre-deploy history, so the reconciliation must only reconcile windows that opened
-- on/after the date the ledger began capturing every transit posting — which is the
-- date this migration (and the ledger write-sites it ships with) is deployed. Stored
-- as a plain YYYY-MM-DD string (non-sensitive setting → raw value, matching
-- serializeSettingValue). Mirrors cogs_ledger_coverage_start_date (20260623160000).
--
...
VALUES ('transit_ledger_coverage_start_date', to_char(CURRENT_DATE + INTERVAL '1 day', 'YYYY-MM-DD'), NOW())
